...
## Places and walking in Ask

The restaurant-to-station question exposed two missing adapters: Ask could resolve GTFS stops but not businesses, and its routing tool always requested transit. `place_search` now queries Photon for named places and addresses; `walk_route` calls the same VIGO Route adapter as Studio with walking mode. Transit and Reach also accept returned place IDs. No geocoding, route ranking, or street-search algorithm is implemented in the model.

Search defaults to the loaded City's GTFS stop bounds, with an optional exact stop as a location preference. These bounds describe the loaded network, not a municipal boundary. An explicit `withinCity: false` search can look beyond them. Results retain provider ordering, addresses, OSM identities, coordinates, retrieval times and attribution. Several plausible locations require clarification; a contextual choice must name its address. The server resolves place IDs to returned coordinates before routing, so generated coordinates cannot replace a named result. Search is an incomplete index and cannot establish business hours, open doors, or accessible entrances.

The default endpoint is `https://photon.komoot.io/api/`. [Photon documents its search parameters](https://github.com/komoot/photon/blob/master/docs/api-v1.md) and [permits moderate use of its public demo without an availability guarantee](https://github.com/komoot/photon#demo-server). Set `VIGO_AGENCY_PLACE_SEARCH_URL` to a privately hosted Photon endpoint, or `off` to disable external place lookup. Only the requested place text, network bounds and optional stop focus are sent; the full chat, staff notes, feed URLs and model credentials are excluded. Requests are spaced across City sessions, timed out and byte limited. Timestamped results are cached for 15 minutes, with bounded session memory. This is online place search, not unrestricted web browsing or a complete business directory.

Walking distance and geometry come from VIGO's saved directed pedestrian graph. Time uses its configured walking speed; endpoint connections and graph diagnostics remain in the saved result. The model receives compact distances in metres and miles, duration, endpoint labels and status. The UI presents a walking distance, estimated time and map action without a transit-leg count or invented departure clock. A missing street index or path produces no distance. Routes do not certify public entrance access or current sidewalk conditions.

Focused fixtures cover provider identities, City bounds, multiple branches, invalid coordinates, private endpoints, disabled search, cancellation, cache isolation, response limits, failed routing, exact coordinate handoff and retained map geometry. Live verification in Boston used the existing `qwen3.5:4b` model: the online result identified Chick-fil-A at 569 Boylston Street, independently consistent with the [restaurant's published address](https://chickfilacopleysquare.com/contact-us). The saved street graph returned 448.28 m and 5.60 minutes to Back Bay station, with 28 geometry points, using 4.8 km/h. This is a check of one public example, not a geocoding benchmark or a claim of universal model reliability.
