...
| Nearby stops without explicit transfer | Prepared | A batched Rust one-to-many expansion creates every directed OSM-reachable alight-to-board edge within 500 m. Each edge stores its street fingerprint and routed distance; unproved radial pairs remain audit counts. | Supported within the versioned OSM transfer graph. |
...
Rebuilding a pre-permission SQLite store is mandatory before using it as
release evidence.
