...
| API route response | LRU: 128 responses, 8 MiB | Explicit response-cache algorithm generation; routing and street schema, canonical source fingerprint, complete metadata-generation digest, and storage generation; recursively sorted complete request. | Algorithm-generation change causes a miss; project/feed/store mutation invalidates project entries; a fingerprint or storage-generation change cannot reuse the old response. |
...
| Service-coverage summary | Eight entries | Routing-store path, schema, canonical source fingerprint, complete metadata-generation digest, and storage generation. | Any canonical artifact-identity change. |
