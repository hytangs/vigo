...
General web search now defaults to [DuckDuckGo's non-JavaScript search](https://duckduckgo.com/duckduckgo-help-pages/features/non-javascript), independently of the model and without an API key. The adapter extracts result links and their own snippets; a challenge or unreadable page is a failed search, never evidence of zero matches. This public interface can change or limit requests. In **Ask → Edit AI connection → Web sources**, select [Brave Search](https://api-dashboard.search.brave.com/api-reference/web/search/get) with a separate search API key, or a private [SearXNG endpoint](https://docs.searxng.org/dev/search_api.html), for an alternative. SearXNG must enable JSON results. Wikipedia-only and Off remain explicit options. Search settings apply to the app session; existing explicit environment settings take precedence over the default.

The connection check sends `public transit`; later searches send only their explicit query. Keys stay in server memory, never enter model context, and do not carry across providers. Live checks found official sources for Liuyishou Hotpot, New Golden Gate Seafood and Maruichi. These checks establish working retrieval for those queries, not general model accuracy or incident-cause verification.

Environment configuration is also supported:

- `VIGO_AGENCY_WEB_SEARCH_PROVIDER`: `duckduckgo` (default), `brave`, `searxng`, `wikipedia`, or `off`.
...
Page reads use the existing DNS-pinned HTTP transport, with public-address checks on every redirect, a 20-second deadline and a 2 MB response limit. This admits script-heavy publisher pages while returning at most 14,000 characters of visible text. Scripts are not executed. Google and other recognized search-results URLs direct the model to the actual search tool instead of reporting a successful page read. Search responses are limited to 512,000 bytes, five results and a one-minute cache. Repeated equivalent lookups within a turn reuse their original source. Retrieved text remains evidence, not instructions; retrieval time is distinct from publication time. A failed search does not prevent a draft using known facts.
...
## Walking constraints and complete outings

The Park Street example exposed both routing and agent failures. A station centroid attached to a disconnected street component; separate street workers could also wait indefinitely behind two leased City workers. Walking now compares declared GTFS station entrances and uses the City's existing routing worker. Exact typed station labels and City-qualified names resolve to their GTFS identities. No coordinates are moved to improve a result.

`walk_compare` measures up to six destinations and, when requested, their directed pairwise connections. Minimum-distance checks compare metres against the exact 1,609.344 metres per mile before display rounding. A missing path is unknown. On the retained Boston street graph, walks from Park Street's selected entrances to the original example coordinates measured 0.15 miles to Boston Common, 0.52 to Faneuil Hall and 0.96 to Boston Public Library. All three fail a one-mile minimum. This three-destination API diagnostic took 2.55 seconds; it is not a city-wide accuracy or performance benchmark.

`find_walk` composes one or two ordered category visits, up to five mapped candidates each, and the existing native walking calculation. The origin is a name: an exact indexed identity uses GTFS, otherwise the place resolver retains its ambiguity rules. Visits require an OSM category. An optional query narrows the business/place name; leaving it out searches the category around the origin with the City name. This avoids treating a hotel, piano shop, car park or tennis court as a public park. Explicit private/no-access tags disqualify a candidate; missing tags remain unknown. Selection minimizes measured walking time among the returned combinations. It does not rank business quality or certify current access.

The completed workflow renders its measurements directly, avoiding a final model call just to rewrite numbers. Ordinary conversation and open-ended research still use model prose. The result states remaining time without inventing food queues or activity duration. Selected identities and map details remain available in saved conversation context and survive a server restart.

Place details come from the exact OSM object through the [OpenStreetMap API](https://wiki.openstreetmap.org/wiki/API_v0.6). `VIGO_AGENCY_PLACE_DETAILS_URL` can select a compatible private endpoint or `off`; the default is `https://api.openstreetmap.org/api/0.6/`. Disabling place search also disables these reads. Requests have a timeout and byte limit, retain only relevant tags, and cache details for 15 minutes. Runtime facts list this network capability separately. Local model inference does not imply that place lookup stays local.

[Retained replays](evidence/agency-walking-audit.json) include the initial incomplete answer, later completed walks and their exact arguments/timers. The final replay found Dave's Hot Chicken on Stuart Street followed by Boston Common: 1,136 metres (0.71 miles), displayed as about 15 minutes walking and 25 minutes left from a 40-minute budget. It completed in one model call: 27.24 seconds of model time plus 3.58 seconds of tool time, 30.83 seconds in the agent loop. Earlier completed runs needed retries and took 72 and 109 seconds. These are evolving diagnostic runs, not a matched before/after benchmark. Earlier attempts confused parking and tennis courts with parks or stopped before synthesis; these failures drove the category and input changes. The evidence does not establish GPT-level general intelligence or consistent performance across arbitrary queries. No model above the prototype's roughly 6B ceiling was used in this follow-up.

## A particular vehicle at a downstream station

`stop_arrivals` accepts a vehicle number separately from the destination station and route. Ask first chooses a station board or a specific vehicle lookup. The latter requires both vehicle and destination fields; an earlier form made the vehicle optional, and the 4B model still put “SL1” in the station field. Exact route display names resolve through the timetable.

The lookup matches the public fleet label to the reported trip and service date before filtering by destination. A fresh position takes precedence over advance trip assignments. Without a position, multiple assignments remain unresolved. The shared station board supplies separate arrival/departure predictions and schedules; missing, stale, conflicting, skipped or cancelled reports never become a confirmed ETA. Completed results render directly, with no model rewrite.

A [retained live diagnostic](evidence/agency-vehicle-arrival.json) answered the original 1340/South Station question with Qwen3.5 4B in one model call, 19.86 seconds total (0.79 seconds in the tool). The observed answer predicted 22:09 EDT against a 22:08 schedule. This is dated evidence, not a current arrival announcement or general latency benchmark. Regression fixtures cover an earlier unrelated bus, future assignments, absent GPS, ambiguous fleet labels, missing arrival predictions, cancellations, skipped stops, passed calls, terminal arrivals, parent stations and overnight service dates.

## Fleet, terminal departure and cycle questions

`service_timing` exposes three explicit choices from the existing trip and line-view data: fresh vehicle positions, a vehicle’s reported trip terminals, and scheduled running time out and back. This separates a fleet list from advance TripUpdate assignments, a past terminal question from an upcoming station board, and cycle time from headway. The result is rendered directly. The cycle comparison pairs matching terminal identities and excludes layovers; it does not claim a measured vehicle cycle. Retained terminal predictions remain forecasts, even after their stated departure time.

The [three-question diagnostic](evidence/agency-service-timing.json) uses the reported Route 9 questions and local Qwen3.5 4B. Each final run used one model call; the fleet and cycle questions took approximately 11 and 10 seconds, and the revised terminal question took 21 seconds. An earlier terminal attempt repeatedly requested a generic “Terminal” station, so the final instructions and error response explicitly direct terminal questions to the vehicle’s own trip. These snapshots are dated evidence. The session-only remote Qwen3.5 Flash connection was not retested.

Provider text containing a closing reasoning marker without its opening marker is now filtered before display, export or conversation reuse. Numeric strings are converted only for schema-declared numeric arguments; normal bounds and unknown-field checks still apply. Fixtures cover stale and duplicated vehicle reports, unknown delay, cancelled trips, terminal forecast history, unmatched return terminals, missing service, direct rendering and malformed provider output.

## Realtime journey ranking

Realtime journeys now use the same ordering as scheduled journeys: earliest arrival, then fewer boardings, then less walking. A query-local timetable retains only departures within the known arrival bound and reuses the native Pareto solver. It preserves replaced/cancelled trips, boarding permissions and transfer minima. Matrix and scenario callers do not run this additional ranking. If certification fails, the live arrival result is retained and the diagnostic records the failure. Skipped-stop updates disable boarding and alighting instead of reverting the trip to its timetable, including updates identified only by a feed-local stop ID.

The [Boston boarding comparison](evidence/realtime-journey-quality.json) reproduces the Park Street/Boylston case with saved GTFS/OSM and a controlled zero-delay update. Boylston reaches the same destination at 23:48 with 3.4 minutes less walking. The original realtime payload was not retained; this comparison isolates boarding selection, not global optimality under that historical feed. Twenty warm local samples measured a 2.4 ms median for the added ranking step. Regression checks include 244 exhaustive small-network comparisons and the routing suite; these establish tested engine behavior, not the accuracy of live feed predictions.

## Shared routing and follow-up evidence — September 15, 2026

Ask and the Route panel both call `runNationalRoute` through the same resident native engine. The transfer, dwell and realtime fixes therefore apply to both. Ask does not choose another boarding stop after the engine returns a journey. Routing and Reach performance measurements remain in [the routing report](ROUTING-PERFORMANCE.md); this change does not alter either solver.

Journey results now retain selected legs in model context and saved conversations, including exact seconds, directed transfer provenance, per-leg schedule/prediction status and unverified station access. Previously the multi-mode projection omitted the legs, and saved Ask history retained requests without their computed itineraries. Map geometry and unrelated updated-trip identifiers remain in the full record rather than expanding model context. Old citation numbers are removed when replaying prose; they belong to their original answer.

Local tool forms keep portable date/time patterns during decoding, put the service date before the clock field, and explain malformed arguments. The snapshot date is separated from timetable calendar bounds: today's realtime observation does not mean only today's timetable is available. Ordinary directions can finish with the computed display after one model call. Additional analysis remains available; changed journey conditions require another calculation. The local per-request timeout is 90 seconds, with cancellation and activity updates preserved; OpenAI-compatible requests retain 45 seconds. An explicit environment override takes precedence. A longer timeout prevents premature interruption but does not make inference faster.

`test/check-agency-routing-integration.mjs` runs the real registry, native solver and saved-conversation boundary. Its synthetic bus/train interchange checks 180 versus 181 seconds, an earlier alighting stop, delayed buses, stale observations, a no-transfer request, exact transfer SQL and both result shapes. Provider decisions in the default suite are scripted; they do not establish language-model accuracy. To test the actual local model, set `VIGO_TEST_ASK_MODEL=qwen3.5:4b` and optionally `VIGO_TEST_ASK_DATE` to a weekday before running that file. These opt-in runs use a synthetic City and the configured local endpoint, with no online place search.

The [retained final 4B run](evidence/ask-routing-integration.json) completed all three checks: directions, a saved-transfer explanation and a revised no-transfer calculation. Each used one model call; the first and third executed the native route tool, while the explanation used saved evidence. End-to-end times were approximately 45, 57 and 37 seconds on the shared laptop. Native calls took approximately 21 and 5 milliseconds. These are diagnostics, not evidence of generally fast inference. The record also retains an intermediate failure: an explanation announced an unrequested calculation and reused an invalid citation. The final replay corrected those specific failures; arbitrary questions and model-written causal explanations remain unverified.

## OpenAI-compatible provider check — September 15, 2026

A user-authorized temporary API was tested with `qwen3-omni-flash` and `qwen3.5-flash`. Both passed the function-calling connection probe in about one second. The fixture uses explicit synthetic station names because one model reasonably interpreted the earlier names “Origin” and “Destination” as placeholders. The live test now accepts `VIGO_TEST_ASK_PROTOCOL=openai`, `VIGO_TEST_ASK_URL` and `VIGO_TEST_ASK_KEY`; normal checks still make no model requests. Its logging also handles null assistant content during native tool calls. Credentials are not included in the records.

The test exposed a real argument-format incompatibility: Qwen3.5 Flash sent the nested `when` object as a JSON string. The normalizer now decodes schema-declared objects and arrays before strict validation, preserving valid literal strings and rejecting malformed, oversized, unknown-field and invalid-date inputs. This is a general representation repair, with no place-name or provider-specific logic.

In the [final provider replays](evidence/provider-routing-check.json), both models completed the three tool/coverage checks. Omni took 2.2, 7.3 and 4.5 seconds; Qwen3.5 took 3.1, 4.9 and 4.1 seconds. These are individual diagnostic observations. **Neither passed the manual check of the explanation's arithmetic:** the actual 123-second wait became 126 seconds in Omni's prose and three minutes in Qwen3.5's. Both also exposed unnecessary internal fields; Omni recalculated the saved journey unnecessarily. The native itinerary, requested date and zero-transfer calculation were correct. These results establish API/tool compatibility after the fix, not dependable expert prose or general query accuracy.

## Broader query audit — September 15, 2026

The [60-question audit](ASK-AUDIT-2026-09-15.md) expands this check to GTFS analysis, GTFS-RT semantics, particular vehicles, occupancy, operational advice and conversation follow-ups. Actual cloud-model runs exposed dropped history and requested-window evidence, premature completion after supporting board lookups, and private-draft handling defects; those pipeline defects were repaired. Basic query retests improved. However, wrong route identity, unsupported causes/forecasts, fleet assumptions and incomplete comparisons remain in the [retained answers](evidence/ask-broad-audit.json). Ask is not yet reliable for expert operational judgment; tool success alone is insufficient.
