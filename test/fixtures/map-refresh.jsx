...
export async function runMapRefreshChecks() {
  ...
  try {
    ...
    await wait(() => document.querySelector('.map-stop-tooltip'))
    const tooltip = document.querySelector('.map-stop-tooltip')
    check(tooltip?.textContent.includes('Vehicle V') && tooltip.textContent.includes('Route R') && tooltip.textContent.includes('Trip T'), 'Vehicle hover shows basic identity without clicking or fetching details: ' + tooltip?.textContent)
