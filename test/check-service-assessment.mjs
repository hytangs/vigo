...
try {
  ...
  const priorities = await get([{ kind: 'network' }], ['conditions', 'spacing', 'causes'])
  assert.deepEqual(priorities.sections.map(section => section.check), ['conditions', 'causes'], 'Network conditions already include spacing; do not repeat the same comparisons as another section')
  assert.equal(priorities.presentationVersion, 2)
  assert.match(renderAssessment(priorities), /### Service to check/)
  assert.match(priorities.sections[0].text, /\*\*66\*\*.*reported cancelled/)
  assert.ok((priorities.sections[0].text.match(/^- \*\*/gm) || []).length <= 3, 'The first answer is a bounded route watchlist, not a route inventory')
  assert.match(priorities.sections[0].text, /remains? unknown/)
  assert.match(priorities.sections[0].text, /Passenger impact has not been ranked/)
  assert.equal(priorities.inspections[0].priorityEvidence[0].routeId, '66', 'Exact prioritized measurements remain in the retained record even when the bounded inspection examples differ')
