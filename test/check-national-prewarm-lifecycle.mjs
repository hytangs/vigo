...
function storeKey(projectId) {
  return fixtureStorePath(projectId)
}
...
try {
  ...
  assert.equal(beforeDelete.routingRuntime.responseCache, undefined)
  ...
  assert.equal(afterDistinctRoute.routingRuntime.responseCache, undefined)
  ...
  const repeatedOutsideCoverageRoute = await routeProject(apiUrl, projectIds[6], {
    serviceDate: '2028-07-13',
    serviceDay: 'weekday',
  })
  assert.notEqual(repeatedOutsideCoverageRoute.plan.id, outsideCoverageRoute.plan.id)
  ...
  assert.equal(outsideCoverageWorker?.operationCounts?.route, 2)
  assert.equal(outsideCoverageRuntime.responseCache, undefined)
