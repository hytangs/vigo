...
try {
  ...
  assert.deepEqual(cluster.events.filter(event => event.type === 'bunching' && !event.evidence.comparisonBasis).map(event => event.evidence.observedHeadwaySeconds).sort((a, b) => a - b), [50, 100], 'Retain the original scheduled comparisons in an overtaking cluster')
  assert.deepEqual(cluster.events.find(event => event.evidence.comparisonBasis === 'reordered-predictions').evidence.tripIds, ['T1', 'T3'], 'Also recognize neighbors in the changed running order')
