...
const entryOnlyCache = new WeightedLruCache({ maxEntries: 2 })
entryOnlyCache.set('a', null)
entryOnlyCache.set('b', { value: 2 })
assert.equal(entryOnlyCache.get('a'), null)
entryOnlyCache.set('c', { value: 3 })
assert.equal(entryOnlyCache.get('b'), undefined)
assert.equal(entryOnlyCache.size, 2)

console.log(JSON.stringify({ check: 'national-trip-cache', ...cache.snapshot() }, null, 2))
