...
} from '../src/server/national-route-choices.mjs'
import { stitchNationalAlternativePlans } from '../src/server/national-gtfs-store.mjs'
...
assert(qualityChoices.some((choice) => choice.id === relevantAccessible.id),
  'A complex journey must survive when it materially reduces walking for a rider with limited walking range.')
assert.equal(
  nationalChoiceSupportedBurdenWeights(
    nominalLeastWalk,
    [fastReference, nominalLeastWalk, relevantAccessible],
