...
assert.equal(form.format.anyOf[0].properties.task, undefined, 'Initial decisions do not spend tokens restating the question')
...
assert.equal(form.format.anyOf[0].properties.arguments.properties.departTime.pattern, route.parameters.properties.departTime.pattern, 'Time format stays enforced during local decoding, not merely rejected after generation')
assert.equal(form.format.anyOf[0].properties.arguments.properties.serviceDate.pattern, route.parameters.properties.serviceDate.pattern)
assert.throws(() => form.parse(JSON.stringify({ action: 'route_plan', arguments: { origin: 'Museum', destination: 'Airport', departTime: '2026-08-21T08:00' } })), /Expected Local HH:MM/, 'Other providers receive an actionable format correction')
