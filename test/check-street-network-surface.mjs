...
try {
  ...
  const unboundedDetail = streetNetworkTravelTimeRaster(storePath, {
    ...common,
    includeEdges: true,
    edgeDetailLimit: 0,
    seeds: [{ coordinate: [-71, 42], durationMinutes: 0 }],
  ...
  assert.equal(
    unboundedDetail.diagnostics.edgeDetailTruncated,
    false,
    'The explicit full-map street geometry mode must not truncate reached edges.',
  )
  assert.equal(
    unboundedDetail.edges.length,
    unboundedDetail.diagnostics.reachedEdgeCount,
