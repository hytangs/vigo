...
try {
  ...
  const night = realtimeFixture([tripUpdate('NIGHT', 0, { timestamp: midnight, vehicleLabel: 'Night bus', stopTimeUpdates: [{ stopId: 'B', stopSequence: 20, arrival: { time: midnight + 600 } }] })])
  night.feeds[0].feedTimestamp = midnight
  const nightBoard = stopBoard(context, night, { stopId: 'station', vehicleId: 'Night bus', event: 'arrival' }, midnight)
  assert.equal(nightBoard.vehicle.serviceDate, '2026-09-13', 'Vehicle lookup preserves the reported GTFS service day across midnight')
  assert.equal(nightBoard.rows[0].arrival.current, midnight + 600, 'A parent-station request finds this vehicle at its boarding platform')
  assert.equal(nightBoard.rows[0].stopId, 'B')
  const noCall = stopBoard(context, night, { stopId: 'C', vehicleId: 'Night bus', event: 'arrival' }, midnight)
