...
export function AgencyProviderSettings({ endpoint, provider, onChange, actions }: { endpoint: string; provider: ProviderState; onChange: () => void; actions?: ReactNode }) {
  ...
  return <section className="agency-ai-connection" aria-label={actions ? 'Conversation controls' : 'AI connection'}>
    ...
    {open ? <form id="agency-ai-settings" className="agency-ai-form" onSubmit={(event) => { event.preventDefault(); void submit('connect') }}>
      ...
      <label>Connection type<select value={protocol} disabled={!!busy} onChange={(event) => { setProtocol(event.target.value as 'openai' | 'ollama'); setReasoningEffort(event.target.value === 'ollama' ? 'none' : '') }}><option value="openai">OpenAI-compatible</option><option value="ollama">Ollama</option></select></label>
      ...
      {protocol === 'ollama' ? <p className="agency-caption">Off is recommended for quick answers. Provider default may enable extended thinking. Location choices and connection tests always use a short response with reasoning off.</p> : null}
