...
export function AgencyProviderSettings({ endpoint, provider, onChange }: { endpoint: string; provider: ProviderState; onChange: () => void }) {
  ...
  function show() { setBaseUrl(provider.baseUrl || 'https://api.openai.com/v1'); setModel(provider.model || ''); setReasoningEffort(provider.reasoningEffort || ''); setApiKey(''); setModels([]); setError(''); setMessage(''); setOpen(true) }
  function preset(url: string, effort = '') { setReasoningEffort(effort); setBaseUrl(url); setModels([]); setModel(''); setApiKey(''); setError(''); setMessage('') }
  async function submit(action: 'models' | 'connect' | 'disconnect') {
    if (busy) return
    const controller = new AbortController(); abort.current = controller
    setBusy(action); setError(''); setMessage('')
    try {
      const result = await apiJson<ProviderState & { models?: string[] }>(endpoint, { method: 'POST', signal: controller.signal, body: JSON.stringify({ action: `provider-${action}`, connection: { baseUrl, model, apiKey, reasoningEffort } }) })
  ...
  return <section className={`agency-ai-connection ${open ? 'is-open' : ''}`} aria-label="AI connection">
    <div className="agency-provider-note"><span className={`agency-status-dot ${provider.available ? 'is-ready' : ''}`} /><div><strong>{provider.available ? provider.model : 'Connect a model to Ask'}</strong><p>{provider.available ? `${provider.testedAt ? 'Function calling verified' : 'Configured on server'} · ${provider.source === 'session' ? 'This app session' : 'Environment settings'}` : 'Use your AI provider or a model running on this computer.'}</p></div><button className="agency-text-button" onClick={open ? () => { if (!busy) { setOpen(false); setApiKey('') } } : show} aria-expanded={open} aria-controls="agency-ai-settings">{open ? <X size={15} /> : provider.available ? <Settings2 size={15} /> : <>Connect<ArrowRight size={13} /></>}<span className="sr-only">{open ? 'Close AI settings' : provider.available ? 'Edit AI connection' : ' AI provider'}</span></button></div>
    {open ? <form id="agency-ai-settings" className="agency-ai-form" onSubmit={(event) => { event.preventDefault(); void submit('connect') }}>
      <div className="agency-ai-presets" aria-label="Provider shortcuts"><button type="button" onClick={() => preset('https://api.openai.com/v1')} disabled={!!busy}>OpenAI</button><button type="button" onClick={() => preset('http://localhost:11434/v1', 'none')} disabled={!!busy}>Ollama</button><button type="button" onClick={() => preset('http://localhost:1234/v1')} disabled={!!busy}>LM Studio</button><button type="button" onClick={() => preset('')} disabled={!!busy}>Custom</button></div>
      ...
      <label>Reasoning effort<select value={reasoningEffort} disabled={!!busy} onChange={(event) => setReasoningEffort(event.target.value)}><option value="">Provider default</option><option value="none">Off · faster tool calls</option><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select></label>
