...
export function AgencyProviderSettings({ endpoint, provider, onChange }: { endpoint: string; provider: ProviderState; onChange: () => void }) {
  ...
  return <section className={`agency-ai-connection ${open ? 'is-open' : ''}`} aria-label="AI connection">
    ...
    {open ? <form id="agency-ai-settings" className="agency-ai-form" onSubmit={(event) => { event.preventDefault(); void submit('connect') }}>
      <div className="agency-ai-presets" aria-label="Provider shortcuts"><button type="button" onClick={() => preset('https://api.openai.com/v1')} disabled={!!busy}>OpenAI</button><button type="button" onClick={() => preset('http://localhost:11434', 'none', 'ollama')} disabled={!!busy}>Ollama</button><button type="button" onClick={() => preset('http://localhost:1234/v1')} disabled={!!busy}>LM Studio</button><button type="button" onClick={() => preset('')} disabled={!!busy}>Custom</button></div>
      ...
      <label>Reasoning effort<select value={reasoningEffort} disabled={!!busy} onChange={(event) => setReasoningEffort(event.target.value)}><option value="">Provider default</option><option value="none">Off · faster tool calls</option>{protocol === 'ollama' ? <option value="high">On</option> : <><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></>}</select></label>
