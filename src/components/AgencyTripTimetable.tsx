...
type Trip = { id: string; directionId: string | null; destination: string; departure: number | null; arrival: number | null; added?: boolean }
type Timetable = { timezone: string; trips: Trip[]; trip: (Trip & { serviceDate: string; status: string; predictionAt: number | null; sourceUrl?: string | null; calls: Array<{ stop: { id: string; name: string }; index: number; sequence?: number | null; status: string; progress?: string | null; lastPrediction?: Partial<Record<'arrival' | 'departure', { time: number; observedAt: number | null }>>; arrival: VehicleTiming['arrival']; departure: VehicleTiming['departure'] }> }) | null }
...
export function AgencyTripTimetable({ projectId, routeId, initialTripId = '', initialServiceDate = '' }: { projectId: string; routeId: string; initialTripId?: string; initialServiceDate?: string }) {
  const retained = useRef<{ key: string; source?: string | null; calls: Map<string, NonNullable<NonNullable<Timetable['trip']>['calls'][number]['lastPrediction']>> }>({ key: '', calls: new Map() })
    ...
    async function refresh() {
      ...
      try {
        ...
        if (!controller.signal.aborted && result.trip) {
          ...
          if (!['Predictions available', 'Scheduled only', 'Stale report', 'Added service', 'Added service · position only'].includes(trip.status)) retained.current.calls.clear()
          for (const call of trip.calls) {
            const callKey = JSON.stringify([call.stop.id, call.sequence ?? call.index])
            if (call.status) { retained.current.calls.delete(callKey); continue }
            const previous = retained.current.calls.get(callKey) || {}
            ...
            retained.current.calls.set(callKey, previous)
  ...
  return <section className="agency-trip-timetable" aria-label="Trip timetable">
    <div className="agency-trip-controls"><label>Service date<input type="date" value={date || data?.trip?.serviceDate || ''} onChange={event => { setDate(event.target.value); setSelection('') }} /></label>
      <label>Trip<select value={selection || data?.trip?.id || ''} onChange={event => setSelection(event.target.value)} disabled={!data?.trips.length}>
        {!data?.trips.length ? <option value="">No trips</option> : data.trips.map(trip => <option key={trip.id} value={trip.id}>{trip.added ? 'Added service · ' : ''}{clock(trip.departure)} → {trip.destination} · {trip.id.split('\u001f').at(-1)}</option>)}
      </select></label>
    ...
    {error ? <p role="alert">{error}</p> : !data ? <p role="status">Loading timetable…</p> : !data.trip ? <p>No scheduled or reported added trips for this service date.</p> : <>
      <h2>{data.trip.added ? 'Reported stops to' : 'To'} {data.trip.destination}</h2>{data.trip.added ? <p className="agency-caption">Added service · only reported stops are shown. No scheduled times or delay comparison.</p> : null}<p>{data.trip.status} · {data.trip.serviceDate} · {data.timezone}</p>
