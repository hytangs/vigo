...
export function AgencyComposer({ question, busy, onChange, onSubmit, onStop }: {
  ...
  return <form className="agency-question-form" onSubmit={event => { event.preventDefault(); onSubmit() }}>
    ...
    <textarea id="agency-question" placeholder="Ask about service, routes or a journey…" value={question} onChange={event => onChange(event.target.value)} maxLength={2000} rows={2} onKeyDown={event => {
