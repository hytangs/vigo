  ...
  return <section className="stop-arrival-board" aria-label={`Arrivals at ${data.stop.name}`}>
    ...
    {!rows.length ? <p className="stop-board-empty">No timed service is available here in the next hour.</p> : <ol className="stop-board-list">{rows.slice(0, expanded ? undefined : 8).map(row => {
      ...
      return <li key={row.key} className={notRunning ? 'is-not-running' : ''}>
        ...
        <div className="stop-board-time"><strong>{refreshError && !notRunning ? time(row.expected) : arrivalLabel(row, now, data.timezone)}</strong><small>{row.kind === 'departure' && !notRunning ? 'Departs · ' : ''}{row.status === 'live' ? <><Radio size={10} aria-hidden="true" /> {refreshError ? 'Recorded prediction' : `Prediction ${time(timing.current)}`}</> : row.atStop && !refreshError ? 'Reported at this stop' : notRunning ? time(timing.scheduled) : 'Schedule only'}</small></div>
