...
export function ServiceStateControl({
  ...
  return (
    <div aria-label={mode === 'schedule' ? 'Schedule playback: estimated positions from GTFS stop times' : 'Live vehicle positions'} className={classNames('service-state-panel', mode === 'live' ? 'is-live' : 'is-schedule', playbackRunning && 'is-playing')}>
      ...
      {mode === 'live' ? <details className="service-indicator-legend"><summary>Delay, spacing &amp; crowding key</summary><p>VIGO alert thresholds use current predictions.</p><p>Blue inner ring: predicted delay ≥5 minutes; thicker at ≥15 minutes. Delay is separate from spacing.</p><p>Compressed spacing: amber at ≤half scheduled and ≥3 minutes compression; red at ≤25%, ≤2 minutes apart and ≥5 minutes compression.</p><p>Wider gaps — amber: predicted gap ≥1.5× scheduled and ≥5 min extra. Red: ≥3× scheduled, ≥20 min total and ≥10 min extra.</p><p>Purple outer ring: reported crowded, full, or not accepting passengers. Labels show the reported category. No ring can mean missing data.</p><p>Gaps are predictions at a reference stop; open the vehicle for location and direction.</p></details> : null}
      {mode === 'schedule' ? (
        <>
