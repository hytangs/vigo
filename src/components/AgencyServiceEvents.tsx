...
export function AgencyServiceEvents({ state, ready, filter, onFilter, onSelect }: {
  ...
  return <details className="agency-service-events" open>
    ...
    <div className="agency-event-list">{events.slice(0, limit).map(event => <button key={event.id} className={`agency-event is-${event.severity}`} data-severity={event.severity} onClick={() => onSelect(event)}>
      <span className="agency-event-mark"><Activity size={15} aria-label={event.severity} /></span><span><strong>{event.title}</strong><small>{event.routeId ? state.routes.find(route => route.id === event.routeId)?.name || shortId(event.routeId) : 'Network'}{event.stopId ? ` · ${state.stopNames?.[event.stopId] || shortId(event.stopId)}` : ''}{event.evidence.delaySeconds != null ? ` · ${event.evidence.delaySeconds > 0 ? '+' : ''}${minutes(event.evidence.delaySeconds)} predicted` : event.evidence.observedHeadwaySeconds != null ? ` · ${minutes(event.evidence.observedHeadwaySeconds)} apart / ${minutes(event.evidence.scheduledHeadwaySeconds!)} scheduled` : ''}</small></span><ChevronRight size={15} />
    </button>)}</div>
