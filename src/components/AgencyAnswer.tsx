...
export function AgencyToolOutput({ result, onSelectEvent, onOpenEntry, onResult }: { result: ToolResult; onResult?: (result: ToolResult) => void; onSelectEvent?: (event: OperationalEvent) => void; onOpenEntry?: (id: number) => void }) {
  ...
  if (data?.events) return <div className="agency-answer-events">{data.events.slice(0, 8).map((event) => <button key={event.id} onClick={() => onSelectEvent?.(event)}><div><strong>{event.routeName ? `${event.routeName} · ` : ''}{event.title}</strong><span>{event.scopeDescription || event.stopName || (event.type === 'service-alert' ? 'Published by the agency' : 'Current service report')}</span>{event.evidence.observedHeadwaySeconds != null ? <p><b>{Number((event.evidence.observedHeadwaySeconds / 60).toFixed(1))} min apart</b><span>{event.evidence.comparisonBasis ? 'Local scheduled interval' : 'Scheduled'}: {Number(((event.evidence.scheduledHeadwaySeconds ?? 0) / 60).toFixed(1))} min</span></p> : event.evidence.delaySeconds != null ? <p>{Number((event.evidence.delaySeconds / 60).toFixed(1))} min later than scheduled</p> : null}</div><ArrowRight size={15} /></button>)}</div>
