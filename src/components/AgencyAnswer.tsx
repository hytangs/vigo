...
export function AgencyToolOutput({ result, onSelectEvent, onOpenEntry }: { result: ToolResult; onSelectEvent?: (event: OperationalEvent) => void; onOpenEntry?: (id: number) => void }) {
  const data = result.data as { entries?: Array<{ id: number; title: string; observedAt: string; excerpt: string; notes: string; shortened: boolean }>; rows?: Record<string, unknown>[]; matches?: Array<{ kind: string; id: string; name: string; address?: string; url?: string; title?: string; excerpt?: string }>; plan?: RoutingPlan; events?: Array<OperationalEvent & { routeName?: string; stopName?: string }>; counts?: Record<string, number>; summary?: { maximumCutoffMinutes: number; transitStatus?: { reachedStops: number } } }
  ...
  if (data?.matches) return <div className="agency-entity-results">{data.matches.map((item) => <div key={`${item.kind}/${item.id}`}><strong>{item.name}</strong><span>{item.address || `${item.kind} · ${item.id}`}</span></div>)}</div>
