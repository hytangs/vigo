...
export function AgencyWebSettings({ endpoint, state, onChange }: { endpoint: string; state: NonNullable<ProviderState['web']>; onChange: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      setApiKey(''); onChange(); setMessage(provider === 'off' ? 'Web search is off.' : 'Search connection verified.')
  ...
  return <details className="agency-web-settings"><summary>Web sources · {state.provider === 'wikipedia' ? 'Public reference' : state.searchAvailable ? 'Search connected' : state.readAvailable ? 'Public pages available' : 'Not connected'}</summary>
    <form className="agency-ai-form" onSubmit={event => { event.preventDefault(); void save() }}>
      <p className="agency-caption">Agency alerts are available with your feeds.{state.readAvailable ? ' Ask can also read public page links.' : ''} Wikipedia lookup needs no key. Connect a broader search provider for news and other sources.</p>
      <label>Web search<select value={provider} disabled={busy} onChange={event => { setProvider(event.target.value as typeof provider); setApiKey(''); setMessage('') }}><option value="wikipedia">Wikipedia · no key needed</option><option value="off">Off</option><option value="brave">Brave Search</option><option value="searxng">SearXNG · private server</option></select></label>
