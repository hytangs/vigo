...
export function AgencyPanel({ projectId, snapshot, realtimeRequest, realtimeMessage, realtimeLoading, onConnect, onDisconnect, onLocate, onResult, onOpenData, mapOpen, onToggleMap }: {
  ...
  const refresh = useCallback(async (signal?: AbortSignal) => {
    ...
    try { const next = await apiJson<AgencyState>(`${endpoint}?${new URLSearchParams({ routeId: routeFilter, eventType: eventFilter })}`, { signal }); if (generation === refreshGeneration.current && !signal?.aborted) { setState(next); setObservationError('') } }
