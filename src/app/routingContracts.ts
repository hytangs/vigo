...
import type { RoutingPlan, RoutingResultStatus, RoutingTravelMode } from '../routingModel'
...
const routingActivityStatuses: Record<RoutingActivity['kind'], WorkspaceStatus> = {
  idle: 'idle',
  preparing: 'preparing',
  loading: 'preparing',
  ready: 'ready',
...
export type RoutingPlanRuntime = {
  engineMs?: number
  totalMs?: number
  label: string
  title: string
...
export function routingServiceCoverageDetail(coverage: RoutingServiceCoverage | null) {
...
export function routingPlanServiceDateDetail(plan: RoutingPlan, requestedServiceDate: string) {
...
export function buildRoutingActivity({
  ...
  routingError: string
  routingErrorStatus?: RoutingResultStatus
