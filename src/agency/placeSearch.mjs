...
async function fetchPlaceJson(url, signal, fetchImpl) {
  ...
  let response
  try {
    response = await fetchImpl(url, { signal: signal ? AbortSignal.any([signal, timeout]) : timeout, redirect: 'error', headers: { Accept: 'application/json', 'User-Agent': 'VIGO (https://github.com/hytangs/vigo)' } })
  } catch (error) {
    signal?.throwIfAborted()
