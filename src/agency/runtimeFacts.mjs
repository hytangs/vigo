...
export function queryRuntimeFacts({ provider, webStatus, placesAvailable, placeEndpoint, generatedAt }) {
  const networkTools = []
...
export function withRuntimeActivity(runtime, trace) {
  ...
  if (names.has('place_search')) for (const name of ['route_plan', 'walk_route', 'reach']) names.add(name)
