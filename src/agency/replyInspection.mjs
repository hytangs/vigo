...
export async function inspectUnverifiedReply({ provider, question, draft, context, assessmentTool, signal }) {
  ...
  if (choice.action === 'clarify') {
    const questions = { trip: 'Which trip? A trip ID, or its route, direction and departure time, will help.', vehicle: 'Which vehicle do you mean? Provide its vehicle number.', route: 'Which route do you mean?', stop: 'Which station or stop do you mean?', location: 'Which location do you mean? Provide the intersection, address or a map point.' }
    const missing = { none: '', assignments: 'I can check service data, but crew and relief assignments require your dispatch roster.', maintenance: 'Fault codes and maintenance clearance are not connected. Dispatch and maintenance must assess vehicle fitness.', passengers: 'Passenger counts and a demand baseline are not connected.' }
    ...
    choice = { action: 'reply', text: missing[choice.missingData] || questions[choice.entity] }
  }
