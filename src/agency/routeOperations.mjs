...
import { addedTripIndex, isAddedTrip } from './addedTrips.mjs'
...
function describe(context, vehicle, now, policy, feeds, coverage, updates, added) {
  const extra = added.get(vehicle)
  const match = extra ? { trip: extra.trip, serviceDate: extra.serviceDate, epoch: serviceEpoch(extra.serviceDate, context.timezone) } : coverage.valid ? context.matchTrip(vehicle, coverage.serviceDate) : { reason: coverage.message }
  ...
  if (vehicle.scheduleRelationship && vehicle.scheduleRelationship !== 'SCHEDULED' && !(extra && isAddedTrip(vehicle))) {
    detail.warnings.push(`Vehicle trip is ${vehicle.scheduleRelationship.toLowerCase().replaceAll('_', ' ')}.`)
    return detail
  }
  if (extra) detail.warnings.push(...extra.warnings)
  const { calls, continuous } = extra ? { calls: extra.calls, continuous: !extra.ambiguous } : tripCalls(context, match.trip.trip_id)
  ...
  const candidates = (updates.get(rawId(vehicle.tripId)) ?? []).filter(update => {
    ...
    const identity = extra ? added.get(update) : context.matchTripIdentity(update, coverage.serviceDate)
  ...
  if (update.scheduleRelationship && update.scheduleRelationship !== 'SCHEDULED' && !(extra && isAddedTrip(update))) {
...
function inputs(context, snapshot, now, policy) {
  ...
  return { feeds, updates, coverage: context.coverage(now), added: addedTripIndex(context, snapshot, (record, vehicle) => fresh(record, feeds, now, policy, vehicle)) }
}
...
export function vehicleDetails(context, snapshot, { vehicleId, sourceUrl }, now = Date.now() / 1000, policy = defaultPolicy) {
  ...
  const { feeds, updates, coverage, added } = inputs(context, snapshot, now, policy)
  return describe(context, vehicles[0], now, policy, feeds, coverage, updates, added)
}
...
export function routeOperations(context, snapshot, { routeId, includeTrips = false, tripId, serviceDate }, now = Date.now() / 1000, policy = defaultPolicy) {
  ...
  for (const vehicle of snapshot?.vehicles ?? []) {
    ...
    const detail = describe(context, vehicle, now, policy, feeds, coverage, updates, added)
  ...
  const patterns = [...topology.patterns]
  for (const extra of added.entries.filter(entry => entry.routeId === routeId && entry.calls.length && !entry.ambiguous)) {
    const stops = extra.calls.map(call => station(context, call.stopId))
    const placements = topology.patterns.flatMap(pattern => {
      if (extra.directionId === null || pattern.directionId !== extra.directionId) return []
      return pattern.stops.flatMap((_, offset) => stops.every((stop, index) => pattern.stops[offset + index]?.id === stop.id) ? [{ pattern, offset }] : [])
    })
    const placement = placements.length === 1 ? placements[0] : null
    let pattern = placement?.pattern
    if (!pattern) {
      const id = `added:${JSON.stringify([extra.directionId, stops.map(stop => stop.id)])}`
      pattern = patterns.find(item => item.id === id)
      if (pattern) pattern.trips++
      else { pattern = { id, directionId: extra.directionId, stops, trips: 1, reportedOnly: true }; patterns.push(pattern) }
    }
    for (const vehicle of vehicles) if (vehicle.tripId === extra.id && vehicle.serviceDate === extra.serviceDate) {
      vehicle.patternId = pattern.id
      if (placement && vehicle.callIndex !== null) vehicle.callIndex += placement.offset
    }
  }
  ...
  return { routeId, name: resolved.short_name || resolved.long_name || rawId(routeId), color: /^[0-9a-f]{6}$/i.test(resolved.color) ? `#${resolved.color}` : 'var(--vigo-lime-strong)',
    ...
    ...(includeTrips ? routeTripTimetable(context, snapshot, routeId, serviceDate || coverage.serviceDate, tripId, now, policy, added, feeds) : {}),
    patterns, vehicles, warnings: coverage.valid ? [] : [coverage.message] }
}
...
function routeTripTimetable(context, snapshot, routeId, serviceDate, tripId, now, policy, added, feeds) {
  ...
  for (const extra of added.entries.filter(entry => entry.routeId === routeId && entry.serviceDate === serviceDate)) {
    const times = extra.current ? extra.calls.flatMap(call => [call.report?.arrival?.time, call.report?.departure?.time].filter(finite)) : []
    trips.push({ id: extra.id, directionId: extra.directionId, destination: extra.calls.length ? station(context, extra.calls.at(-1).stopId).name : 'Unreported destination', departure: times.length ? Math.min(...times) : null, arrival: times.length ? Math.max(...times) : null, added: true })
  }
  trips.sort((a, b) => (a.departure ?? Infinity) - (b.departure ?? Infinity) || a.id.localeCompare(b.id))
  ...
  const extra = added.entries.find(entry => entry.id === selected.id && entry.serviceDate === serviceDate)
  const updates = (snapshot?.tripUpdates ?? []).filter(update => {
    const match = extra ? added.get(update) : context.matchTripIdentity(update, serviceDate)
    return match?.trip?.trip_id === selected.id && match.serviceDate === serviceDate
  })
  const update = updates.length === 1 ? updates[0] : null
  const status = extra?.ambiguous || updates.length > 1 ? 'Unresolved reports' : !update ? extra ? 'Added service · position only' : 'Scheduled only' : !fresh(update, feeds, now, policy) ? 'Stale report'
    : update.scheduleRelationship && update.scheduleRelationship !== 'SCHEDULED' && !isAddedTrip(update) ? update.scheduleRelationship : extra ? 'Added service' : 'Predictions available'
  const usable = status === 'Predictions available' || status === 'Added service'
  const { calls } = extra ?? tripCalls(context, selected.id)
  const vehicles = (snapshot?.vehicles ?? []).filter(vehicle => {
    const identity = extra ? added.get(vehicle) : context.matchTripIdentity(vehicle, serviceDate)
  ...
  const vehicle = vehicles.length === 1 && fresh(vehicles[0], feeds, now, policy, true)
    && (!vehicles[0].scheduleRelationship || vehicles[0].scheduleRelationship === 'SCHEDULED' || extra && isAddedTrip(vehicles[0])) ? vehicles[0] : null
  ...
  return { trips, trip: { ...selected, serviceDate, status, sourceUrl: update?.sourceUrl ?? null,
    ...
    calls: calls.map((call, index) => {
      ...
      return { stop: station(context, call.stopId), index, sequence: call.sequence,
