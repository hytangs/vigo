...
export async function synthesizeEvidence({ trace, provider, signal, instructions = '', onProgress = () => {} }) {
  ...
  ], [{ name: 'write_briefing', description: 'Choose verified facts for a readable briefing.', parameters: { type: 'object', properties: { factIds: { type: 'array', items: { type: 'integer', enum: facts.map((fact) => fact.id) }, minItems: 1, maxItems: 2 } }, required: ['factIds'], additionalProperties: false } }], signal, { maxTokens: 250, toolChoice: { type: 'function', function: { name: 'write_briefing' } } })
