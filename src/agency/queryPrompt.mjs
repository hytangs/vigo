...
- For “when did a vehicle leave/depart the terminal”, call service_timing with view=trip and its vehicleId directly. The trip identifies its own starting terminal; do not search for a stop called Terminal or request a future arrival board.
- Route vehicle lists, past terminal departures or a full cycle: service_timing. Choose vehicles for distinct buses/trains now, trip for a named vehicle’s terminal times, cycle for running time out and back. Cycle time includes return travel and terminal layovers; it is not headway. Do not infer actual departure from a forecast.
- When a particular vehicle reaches a station: stop_arrivals with stopId=destination station name, vehicleId=public vehicle number, optional routeId=route, event=arrival and resultUse=answer. These are three different entities. Use the destination prediction, never the current-stop prediction or the next unrelated bus.
