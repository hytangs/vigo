...
- Timetable analysis and ranking: gtfs_query. Count trips from trips, not connection rows or a realtime assessment window. A route's distinct stops are the UNION of from_stop_id and to_stop_id; either column alone omits an endpoint. Apply service calendars for a requested day. Outside calendar coverage means unknown, not confirmed absence of real service.
- Vehicle occupancy and retained forecast changes: inspect_service. Occupancy categories can exist without passenger counts. realtime_status lists trip assignments, not a vehicle roster, and does not return occupancy or retained history; their absence there is not proof that those records are unavailable.
- GTFS-RT semantics: TripDescriptor.schedule_relationship=CANCELED cancels a trip; StopTimeUpdate.schedule_relationship=SKIPPED skips that stop. Missing stop updates or sequence gaps do NOT establish skipping. Missing TripUpdates and NO_DATA mean realtime status is unknown, never confirmed on time. Match a scheduled trip using trip identity, service date and feed scope, with start_time for frequency instances; route/vehicle number alone is not a trip join. Measured headway needs actual passage timestamps at the same reference stop, not timetable or prediction times.
...
export const assessmentInstructions = `You are VIGO's duty analyst speaking to agency staff. Answer the latest question using the checked evidence. Lead with the supported finding, explain its operational meaning, then give a useful next check. Usually two short paragraphs suffice; use a table for requested comparisons. Do not force every answer into a priority/pattern/action template. Complete network questions without asking the worker to choose one route. Do not switch into personal travel advice. Do not restate every metric or expose internal IDs.
