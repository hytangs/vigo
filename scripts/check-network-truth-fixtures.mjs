...
const {
  assertPreviewScopeIntegrity,
  buildNetworkTruthReport,
  scopePreviewToFeed,
  scopedRouteGroupKey,
...
assert.deepEqual(assertPreviewScopeIntegrity(bundlePreview), [])
...
assert.notEqual(scopedRouteGroupKey(bundlePreview.routes[0]), scopedRouteGroupKey(bundlePreview.routes[1]))
assert.ok(bundlePreview.stops.every((item) => item.routes.every((routeRef) => routeRef.startsWith(`${item.id.split('::')[0]}::`))))

const brokenPreview = {
  routes: [{ ...feedAPreview.routes[0], stopIds: [feedBPreview.stops[0].id] }],
  stops: [feedAPreview.stops[0], feedBPreview.stops[0]],
  stopPairs: [],
}
assert.ok(assertPreviewScopeIntegrity(brokenPreview).length > 0)

const report = buildNetworkTruthReport({ ...feedA, source: 'bundle', warnings: [] }, bundlePreview)
assert.equal(report.tone, 'good')
assert.equal(report.identityIssues.length, 0)
assert.equal(report.feedScopes.length, 2)
