...
} from '../server/national-route-geometry.mjs'
import { buildNativeStreetCchIndex } from '../server/native-routing-kernel.mjs'
import { disposeNationalStopSearchStore, searchNationalGtfsStops } from '../server/national-stop-search.mjs'
...
async function runOversizedSourcePreflightFixture(routingStorePath) {
  const modulePath = path.join(repositoryRoot, 'server', 'national-gtfs-store.mjs')
  const source = `
...
async function runConfiguredEndpointOverheadFixture(routingStorePath, streetStorePath) {
  const modulePath = path.join(repositoryRoot, 'server', 'national-gtfs-store.mjs')
  const osmModulePath = path.join(repositoryRoot, 'server', 'national-osm-store.mjs')
  const source = `
    import { pathToFileURL } from 'node:url'
    const routing = await import(pathToFileURL(process.argv[1]).href)
...
try {
  ...
  const compactKernelCachedAfterRoleAdmission = routeNationalGtfsStore(storePath, {
    origin: { coordinate: [8.0000, 47.0000], label: 'Kernel Alpha origin', source: 'map' },
    destination: { coordinate: [8.0200, 47.0200], label: 'Kernel Charlie destination', source: 'map' },
    departMinutes: 8 * 60,
    serviceDay: 'sunday',
  ...
  assert.equal(
    compactKernelCachedAfterRoleAdmission.diagnostics.searchStats.cacheHit,
    true,
    'Read-only stop-role admission must not evict a resident timetable kernel or its exact route cache.',
  )
  const cacheEntriesBeforeDisabledQuery = compactKernelPlan.diagnostics.searchStats.resultCache.entries
  const compactKernelUncachedPlan = routeNationalGtfsStore(storePath, {
    origin: { coordinate: [8.0000, 47.0000], label: 'Uncached Alpha origin', source: 'map' },
    destination: { coordinate: [8.0200, 47.0200], label: 'Uncached Charlie destination', source: 'map' },
    departMinutes: 8 * 60 + 0.25,
    serviceDay: 'sunday',
    serviceDate: '2026-07-12',
    maxWalkKm: 0.25,
    __disableResultCache: true,
  })
  assert.equal(compactKernelUncachedPlan.status, 'ready')
  assert.equal(compactKernelUncachedPlan.diagnostics.searchStats.resultCachePolicy, 'disabled')
  assert.equal(
    compactKernelUncachedPlan.diagnostics.searchStats.resultCache.entries,
    cacheEntriesBeforeDisabledQuery,
    'Cache-disabled queries must not retain unique route geometry.',
  )
  ...
  for (const departureMinutes of [8 * 60, 8 * 60 + 4.5, 8 * 60 + 5, 8 * 60 + 5.25]) {
    const parityRequest = {
  ...
  assert(
    denseAccessArriveBy.diagnostics.searchStats.arriveByVerifiedCandidates > 1,
    'The dense fixture must reject a later-departing slow trip before verifying the exact winner.',
  )
  assert.equal(
    denseAccessArriveBy.diagnostics.searchStats
      .coordinateAccessFrontierReuses,
    1,
    'Rust must verify the boundary internally and reuse the endpoint frontier only for final materialization.',
  )
  assert.equal(
  ...
  assert(
    ...
    'Exact arrive-by verification count must remain logarithmic in the complete candidate set.',
  )
  ...
  const quantizedDeadlineArriveBy = routeNationalGtfsStore(
    denseAccessStorePath,
    {
      ...quantizedDeadlineRequest,
      timePreference: 'arrive',
      arriveMinutes: quantizedDeadlineDepartPlan.arriveMinutes,
    },
  )
  ...
  assert.equal(
    ...
    quantizedDeadlineDepartPlan.diagnostics.searchStats.selectedArrivalSeconds,
    'Arrive-by must compare the exact internal second, not a rounded public-minute round trip.',
  )
  assert.deepEqual(
    quantizedDeadlineArriveBy.legs
      .filter((leg) => leg.type === 'ride')
      .map((leg) => [leg.routeId, leg.tripId, leg.startMinutes, leg.endMinutes]),
    quantizedDeadlineDepartPlan.legs
      .filter((leg) => leg.type === 'ride')
      .map((leg) => [leg.routeId, leg.tripId, leg.startMinutes, leg.endMinutes]),
  )
  const oneSecondLaterThanQuantizedBoundary = routeNationalGtfsStore(
    denseAccessStorePath,
    {
      ...quantizedDeadlineRequest,
      departMinutes: quantizedDeadlineArriveBy.departMinutes + 1 / 60,
    },
  )
  assert(
    oneSecondLaterThanQuantizedBoundary.status !== 'ready'
      || oneSecondLaterThanQuantizedBoundary.diagnostics.searchStats
        .selectedArrivalSeconds
        > Math.round(quantizedDeadlineDepartPlan.arriveMinutes * 60),
    'The second immediately after the selected arrive-by boundary must be infeasible for the deadline.',
  )
  ...
  const cachedAnchorProfileWalk = routeNationalGtfsStore(
    ...
    {
      ...shortWalkRequest,
      departMinutes: shortWalkRequest.departMinutes + 0.25,
      __disableResultCache: true,
    },
  )
  assert.equal(
  ...
  assert.equal(arriveByPlan.diagnostics.algorithm, 'rust_exact_arrive_by_monotone_boundary_no_heuristic')
  ...
  assert.match(mergedRawMetadata.sourceFingerprint, /^[a-f0-9]{64}$/)
  assert.deepEqual(
  ...
  assert.equal(explicitCrossFeedArrivePlan.diagnostics.algorithm, 'rust_exact_arrive_by_monotone_boundary_no_heuristic')
  ...
  }), /lacks a 0\.1\.4 source fingerprint/)
  ...
  console.log(`National routing-store fixture passed (${result.buildSeconds}s build / ${plan.diagnostics.searchStats.queryMs}ms query / ${firstRepairMs.toFixed(1)}ms repair / ${secondRepairMs.toFixed(1)}ms warm read).`)
} finally {
