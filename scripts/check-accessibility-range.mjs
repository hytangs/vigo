...
const folder = await fsp.mkdtemp(path.join(os.tmpdir(), 'vigo-accessibility-range-'))
...
try {
  ...
  const progress = []
  const first = routeNationalGtfsAccessibilityRange(storePath, request(), {
    streetStorePath,
    onProgress: (event) => progress.push(event),
  })
  assert.equal(first.schemaVersion, 'vigo.analysis.accessibility-range.v1')
  ...
  assert(first.surface.nodes.length > 0, 'Accessibility must retain exact reached OSM node evidence.')
  ...
  assert.equal(first.diagnostics.surface.owner, 'rust_resident_accessibility_pipeline')
  ...
  const fullGeometry = routeNationalGtfsAccessibilityRange(
    storePath,
    request({
      surface: {
        ...request().surface,
  ...
  assert.equal(fullGeometry.surface.diagnostics.edgeEvidenceLimit, 0)
  assert.equal(fullGeometry.surface.diagnostics.edgeEvidenceTruncated, false)
  ...
  assert.equal(
    ...
    'Accessibility street paths must return every reached directed OSM edge.',
  )
  ...
  const excluded = routeNationalGtfsAccessibilityRange(
    storePath,
  ...
  const overlay = routeNationalGtfsAccessibilityRange(storePath, request({
    scenarioOverlay: {
      stops: [
  ...
  const outsideWindow = routeNationalGtfsAccessibilityRange(
    storePath,
  ...
  const noService = routeNationalGtfsAccessibilityRange(
    storePath,
  ...
  const noServiceWindow = routeNationalGtfsAccessibilityRange(
    storePath,
  ...
  const replay = routeNationalGtfsAccessibilityRange(storePath, request(), { streetStorePath })
  assert.deepEqual(
    replay.stops,
    first.stops,
  ...
  await assert.rejects(
    async () => routeNationalGtfsAccessibilityRange(
      storePath,
  ...
  const sourceFiles = [
    'server/native-routing-kernel.mjs',
    'server/national-route-worker.mjs',
    'server/scenario-analysis.mjs',
  ...
  assert(!fs.existsSync(path.join(repositoryRoot, 'server', 'regional-analysis-kernel.mjs')))
