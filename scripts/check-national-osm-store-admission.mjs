...
try {
  ...
  assertAdmissionRejected(
    await mutatedCopy('malformed-columns', 'ALTER TABLE edges RENAME COLUMN distance_m TO cost;'),
  ...
  assertAdmissionRejected(
    await mutatedCopy('malformed-index', 'DROP INDEX edges_from; CREATE INDEX edges_from ON edges(to_node);'),
    'index_columns_mismatch',
  )
  assertAdmissionRejected(
    await mutatedCopy('malformed-metadata', "UPDATE metadata SET value='not-json' WHERE key='edgeCount';"),
    'metadata_malformed',
  )
  assertAdmissionRejected(
    await mutatedCopy('oversized-metadata', "UPDATE metadata SET value=hex(zeroblob(600000)) WHERE key='edgeCount';"),
