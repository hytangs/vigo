...
try {
  ...
  assert.equal(walk.diagnostics.searchStats.cacheHit, false)
  assert.equal(walk.diagnostics.searchStats.accelerated, true)

  const cachedWalk = routeNationalStreetStore(currentStore, walkRequest)
  assert.equal(cachedWalk.diagnostics.searchStats.cacheHit, true)
  assert.equal(Object.hasOwn(cachedWalk, '_search'), false)
  assert(
    cachedWalk.diagnostics.searchStats.queryMs < walk.diagnostics.searchStats.queryMs,
    'An exact repeated walk must use the faster identity-bound result cache.',
  )
  ...
  const worker = new Worker(new URL('../server/national-route-worker.mjs', import.meta.url))
  ...
  const matrixWorker = new Worker(new URL('../server/national-route-worker.mjs', import.meta.url))
  ...
  const batchWorker = new Worker(new URL('../server/national-route-worker.mjs', import.meta.url))
  ...
  const fallbackWorker = new Worker(new URL('../server/national-route-worker.mjs', import.meta.url))
  ...
  console.log(JSON.stringify({
    ...
    walk: {
      ...
      cachedQueryMs: cachedWalk.diagnostics.searchStats.queryMs,
    },
    drive: {
