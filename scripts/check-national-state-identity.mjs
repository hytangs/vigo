...
import { finalizeCurrentStreetFixture } from './lib/street-fixture.mjs'
...
try {
  ...
  const preferenceCacheRequest = {
    ...request,
    origin: { coordinate: [-70, 41], label: 'Cache policy origin', source: 'stop', stopId: 'CO' },
    destination: { coordinate: [-70.02, 41.02], label: 'Cache policy destination', source: 'stop', stopId: 'CD' },
    __disableResultCache: false,
  }
  const cachedFastest = routeNationalGtfsStore(storePath, {
    ...preferenceCacheRequest,
    routingPreference: 'fastest',
  })
  const cachedBalanced = routeNationalGtfsStore(storePath, {
    ...preferenceCacheRequest,
    routingPreference: 'balanced',
  })
  assert.deepEqual(
    cachedFastest.legs.filter((leg) => leg.type === 'ride').map((leg) => leg.routeShortName),
    ['CF1', 'CF2'],
    'The cache-policy fixture must expose the earlier two-boarding journey.',
  )
  assert.deepEqual(
    cachedBalanced.legs.filter((leg) => leg.type === 'ride').map((leg) => leg.routeShortName),
    ['CB'],
    'A cached fastest result must never leak into a balanced request with the same endpoints and departure.',
  )
  assert.notEqual(
    cachedBalanced.diagnostics.searchStats.cacheHit,
    true,
    'The first balanced request must execute its own policy-specific search.',
  )
  const cachedBalancedReplay = routeNationalGtfsStore(storePath, {
    ...preferenceCacheRequest,
    routingPreference: 'balanced',
  })
  assert.equal(
    cachedBalancedReplay.diagnostics.searchStats.cacheHit,
    true,
    'The policy-specific balanced result should remain independently reusable.',
  )

  let disabledCacheKeyOnlyReads = 0
  const disabledCacheKeyRequest = {
    ...request,
    departMinutes: request.departMinutes + 0.25,
  }
  Object.defineProperty(disabledCacheKeyRequest, 'staticTopologyPath', {
    enumerable: true,
    get() {
      disabledCacheKeyOnlyReads += 1
      return undefined
    },
  })
  const disabledCacheKeyPlan = routeNationalGtfsStore(storePath, disabledCacheKeyRequest)
  assert.equal(disabledCacheKeyPlan.status, 'ready')
  assert.match(disabledCacheKeyPlan.diagnostics.algorithm, /^rust_exact_connection_scan/)
  const dataSemantics = disabledCacheKeyPlan.diagnostics.dataSemantics
  ...
  assert.equal(
    disabledCacheKeyOnlyReads,
    0,
    'A result-cache-disabled route must not construct or inspect the unused full-plan cache key.',
  )

  let enabledCacheKeyOnlyReads = 0
  const enabledCacheKeyRequest = {
    ...request,
    departMinutes: request.departMinutes + 0.25,
    __disableResultCache: false,
  }
  Object.defineProperty(enabledCacheKeyRequest, 'staticTopologyPath', {
    enumerable: true,
    get() {
      enabledCacheKeyOnlyReads += 1
      return undefined
    },
  })
  const enabledCacheKeyPlan = routeNationalGtfsStore(storePath, enabledCacheKeyRequest)
  assert.equal(enabledCacheKeyPlan.status, 'ready')
  assert.equal(
    enabledCacheKeyOnlyReads,
    0,
    'Rust timetable routes must not read the analysis-only static-topology path.',
  )

  const cacheEnabledRequest = { ...request, __disableResultCache: false }
  const cachedPlanBeforeMutation = routeNationalGtfsStore(storePath, cacheEnabledRequest)
  assert.equal(cachedPlanBeforeMutation.arriveMinutes, 510)
  ...
  `).run()
  mutatedStore.close()
  // A changed source identity must discard both the full-plan cache and the
  // resident Rust timetable image; no analysis-only topology rebuild is needed.
  const refreshedPlanAfterMutation = routeNationalGtfsStore(storePath, cacheEnabledRequest)
  assert.equal(
    refreshedPlanAfterMutation.arriveMinutes,
    515,
    'A changed SQLite source must invalidate the resident full-plan cache and active-service state.',
  )
  assert.equal(
  ...
  const cachedStreetPlanBeforeMutation = routeNationalGtfsStore(streetIdentityStorePath, streetIdentityRequest)
  const uncachedStreetPlanBeforeMutation = routeNationalGtfsStore(streetIdentityStorePath, {
    ...streetIdentityRequest,
    departMinutes: streetIdentityRequest.departMinutes + 0.25,
    __disableResultCache: true,
  })
  assert.equal(cachedStreetPlanBeforeMutation.status, 'ready')
  assert.equal(uncachedStreetPlanBeforeMutation.status, 'ready')
  const cachedStreetPlanHit = routeNationalGtfsStore(streetIdentityStorePath, streetIdentityRequest)
  assert.equal(cachedStreetPlanHit.diagnostics.searchStats.cacheHit, true, 'The fixture must prime the full-plan cache before mutating OSM storage.')
  ...
  const uncachedStreetPlanAfterMutation = routeNationalGtfsStore(streetIdentityStorePath, {
    ...streetIdentityRequest,
    departMinutes: streetIdentityRequest.departMinutes + 0.25,
    __disableResultCache: true,
  })
  assert.equal(
    uncachedStreetPlanAfterMutation.status,
    'ready',
    'SQLite mutation alone must not become a pedestrian execution path while the admitted Rust snapshot is unchanged.',
  )
  const cachedStreetPlanAfterMutation = routeNationalGtfsStore(streetIdentityStorePath, streetIdentityRequest)
  assert.equal(
    cachedStreetPlanAfterMutation.status,
    'ready',
    'A cache refresh must continue to use the immutable Rust snapshot rather than a SQLite routing fallback.',
  )
