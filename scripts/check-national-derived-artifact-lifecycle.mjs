...
async function prepareKernelInWalkingPolicy(overrides = {}) {
  ...
  const moduleUrl = new URL('../server/national-gtfs-store.mjs', import.meta.url).href
  const source = `
...
try {
  ...
  const rebuildSource = await fs.readFile(new URL('./rebuild-vigo-project-from-raw.mjs', import.meta.url), 'utf8')
