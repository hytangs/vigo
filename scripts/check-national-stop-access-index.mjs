...
import { buildNativeStreetCchIndex, disposeNativeRoutingKernel } from '../server/native-routing-kernel.mjs'
...
try {
  ...
  const routingSource = await fs.readFile(path.join(import.meta.dirname, '..', 'server', 'national-gtfs-store.mjs'), 'utf8')
  const streetSource = await fs.readFile(path.join(import.meta.dirname, '..', 'server', 'national-osm-store.mjs'), 'utf8')
  const nativeSource = await fs.readFile(path.join(import.meta.dirname, '..', 'server', 'native-routing-kernel.mjs'), 'utf8')
