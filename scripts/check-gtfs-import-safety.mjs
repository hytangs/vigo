...
import {
  ...
  inspectGtfsZip,
  streamGtfsZipCsv,
} from '../server/gtfs-zip-reader.mjs'
...
async function contract(name, callback) {
  ...
  await callback()
  contracts.push({ name, milliseconds: Number((performance.now() - startedAt).toFixed(3)) })
}

try {
  await contract('safe nested GTFS table paths remain supported', async () => {
  ...
  await contract('duplicate recognized table basenames are rejected deterministically', async () => {
  ...
  await contract('unsafe selected entry names are rejected before extraction', async () => {
  ...
  await contract('traversal and control-character table names are rejected', async () => {
  ...
  await contract('selected Unix symlink entries are rejected without extraction', async () => {
    const zip = new JSZip()
  ...
  await contract('invalid UTF-8 is rejected instead of replacement-decoded', async () => {
  ...
  await contract('single-table expanded-size limits reject oversized metadata', async () => {
  ...
  await contract('archive compressed and selected expanded-size limits are enforced', async () => {
  ...
  await contract('logical CSV records are bounded during streaming', async () => {
  ...
  await contract('empty and duplicate CSV headers are rejected', async () => {
  ...
  await contract('CSV header column counts are bounded', async () => {
  ...
  await contract('row and total-row budgets are enforced', async () => {
