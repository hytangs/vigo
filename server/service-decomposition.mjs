...
function coordinate(value) {
  ...
  const longitude = finite(value[0], NaN)
  const latitude = finite(value[1], NaN)
  ...
  for (const routeId of routeIds) {
    ...
    for (const route of analysis?.routes ?? []) {
      ...
      patterns.push({
        ...
        tripCount: Math.max(0, Math.round(finite(route.tripCount))),
        headwayMinutes: Math.max(0, finite(route.headwayMinutes)),
        geometry,
...
export function buildServiceEdgeDecomposition({
  ...
  const options = {
    matchDistanceM: Math.max(10, Math.min(150, finite(matchDistanceM, defaultMatchDistanceM))),
    sampleSpacingM: Math.max(10, Math.min(100, finite(sampleSpacingM, defaultSampleSpacingM))),
    maxPatterns: Math.max(1, Math.min(10_000, Math.floor(finite(maxPatterns, defaultMaxPatterns)))),
    maxFeatures: Math.max(1, Math.min(250_000, Math.floor(finite(maxFeatures, defaultMaxFeatures)))),
  }
