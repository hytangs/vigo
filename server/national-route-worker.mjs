...
function haversineKm(left, right) {
  ...
  const dLat = (Number(right[1]) - Number(left[1])) * latitudeScale
  const dLon = (Number(right[0]) - Number(left[0])) * latitudeScale
  const latitudeA = Number(left[1]) * latitudeScale
  const latitudeB = Number(right[1]) * latitudeScale
  const value = Math.sin(dLat / 2) ** 2
    + Math.cos(latitudeA) * Math.cos(latitudeB) * Math.sin(dLon / 2) ** 2
...
async function prepareStreetStore(request) {
  ...
  const drive = prepareNationalOsmDriveStore(streetStorePath)
  return {
    ...prepared,
    drive,
  }
...
parentPort.on('message', async (message) => {
  ...
  try {
    ...
    } else if (operation === 'accessibility-range') {
      const { routeNationalGtfsAccessibilityRange } = await loadGtfsModule()
      const cancellation = cancelBuffer instanceof SharedArrayBuffer
        ? new Int32Array(cancelBuffer)
        : null
      result = routeNationalGtfsAccessibilityRange(storePath, request, {
    ...
    parentPort.postMessage({
      ...
      error: error instanceof Error
        ? ['accessibility-range', 'street-surface', 'street-matrix'].includes(operation)
          ? error.message
