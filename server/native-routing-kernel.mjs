...
const nativeKernelCandidates = [...new Set([
  ...
  path.resolve(moduleDirectory, '..', 'server', 'vigo-routing-kernel.node'),
...
export function validateCchManifest({ kind, format, sourcePath, manifestPath, structurePath, metrics, nodeCount, edgeCount }) {
  ...
  for (const [label, filePath] of Object.entries(expectedFiles)) {
    ...
    if (
      record?.file !== path.basename(filePath)
      || record?.bytes !== stat.size
      || record?.sha256 !== sha256File(filePath)
    ) {
      throw new Error(`Native CCH manifest digest mismatch for ${label}.`)
    }
  }
  return manifest
}
...
export function routeNativeCoordinateTimetableScalar(storePath, kernel, request) {
  ...
  const diagnostics = {
    ...
    queryMs: result.queryNs / 1e6,
    accessMs: result.accessNs / 1e6,
    timetableMs: result.timetableNs / 1e6,
    ...
    directWalkDistanceKm: Number.isFinite(result.directWalkDistanceM)
      ...
      : null,
    directWalkQueryMs: result.directWalkQueryNs / 1e6,
    directWalkPathChecked: result.directWalkPathChecked,
    directWalkPathQueryMs: result.directWalkPathQueryNs / 1e6,
    directWalkAccessDominates: result.directWalkAccessDominates,
    coordinateQueryMs: endpoints.queryNs / 1e6,
    accessReductionMs: endpoints.accessReductionNs / 1e6,
    originAccessReductionMs: endpoints.originAccessReductionNs / 1e6,
    destinationAccessReductionMs: endpoints.destinationAccessReductionNs / 1e6,
    snapMs: endpoints.snapNs / 1e6,
    originSearchMs: endpoints.originSearchNs / 1e6,
    destinationSearchMs: endpoints.destinationSearchNs / 1e6,
...
export function routeNativeCoordinateTimetableMany(storePath, kernel, request) {
  ...
  const diagnostics = {
    ...
    accessMs: result.accessNs / 1e6,
    timetableMs: result.timetableNs / 1e6,
    coordinateQueryMs: access.queryNs / 1e6,
    accessReductionMs: access.accessReductionNs / 1e6,
    snapMs: access.snapNs / 1e6,
    originSearchMs: access.searchNs / 1e6,
    originCandidates: result.originCandidateCount,
...
export function routeNativeTimetableArriveBy(kernel, request) {
  ...
  return {
    ...result,
    queryMs: result.queryNs / 1e6,
    engineQueryMs: result.engineQueryNs / 1e6,
    configureMs: record.configureMs,
    kernelDiagnostics: record.diagnostics,
  }
}
...
export function routeNativeTimetableOverlayMany(kernel, request) {
  ...
  return {
    ...
    compileMs: result.compileNs / 1e6,
    scanMs: result.scanNs / 1e6,
    transientBytes: result.transientBytes,
    workspaceBytes: result.workspaceBytes,
    queryMs: result.timetable.queryNs / 1e6,
    configureMs: record.configureMs,
    kernelDiagnostics: record.diagnostics,
  }
}
...
export function routeNativeTimetablePareto(kernel, request) {
  ...
  return {
    ...result,
    queryMs: result.queryNs / 1e6,
    corridorMs: result.corridorNs / 1e6,
    forwardMs: result.forwardNs / 1e6,
    reverseMs: result.reverseNs / 1e6,
    roundMs: result.roundNs / 1e6,
    configureMs: record.configureMs,
    kernelDiagnostics: record.diagnostics,
  }
}
...
export function configureNativeRoutingAccessProfile(storePath, profile, options = {}) {
    ...
    if (snapshotPath) {
      try {
        if (fs.existsSync(snapshotPath)) {
          ...
          snapshotWriteMs = written.elapsedNs / 1e6
        }
      } catch (error) {
  ...
  if (snapshotPath && fs.existsSync(snapshotPath)) {
    try {
      ...
      snapshotLoadMs = loaded.elapsedNs / 1e6
    } catch (error) {
...
function candidatesFromNative(record, arrays, role, queryToken) {
  ...
  for (let index = 0; index < memberIndices.length; index += 1) {
    const memberIndex = memberIndices[index]
    const pathMemberIndex = pathMemberIndices[index]
    const target = record.profileMembers[memberIndex]
    const source = record.profileCandidateBases?.[pathMemberIndex]
    if (!target || !source) {
      throw new Error(
    ...
    const candidateKind = candidateKinds[index]
    const linked = candidateKind === 1
    const exact = candidateKind === 2
    const candidate = {
      stop_id: target.stop_id,
      ...
      parent_station: target.parent_station,
      location_type: target.location_type,
      distanceKm: distancesM[index] / 1000,
      accessSeconds: accessSeconds[index],
      exactStopAccess: exact,
      walkSource: exact ? 'coordinate-colocation' : 'osm-rust',
      streetPathVerified: true,
      streetAccessStopId: linked ? source.stop_id : source.streetAccessStopId,
    ...
    if (linked) {
      const fromStopId = record.profileStopIds?.[linkFromStopKeys[index]]
      const toStopId = record.profileStopIds?.[linkToStopKeys[index]]
      const transferredStop = record.profileStops?.[linkToStopKeys[index]]
      if (!fromStopId || !toStopId || !transferredStop) {
        throw new Error('Rust linked-access frontier returned unknown transfer stop keys.')
      }
      const transferCoordinate = [transferredStop.lon, transferredStop.lat]
      Object.assign(candidate, {
        accessTransferFromStopId: fromStopId,
        accessTransferToStopId: toStopId,
        accessTransferSeconds: linkDurations[index],
        accessTransferCoordinate: transferCoordinate,
        ...(linkPathDistancesM[index] >= 0
          ? { accessTransferPathDistanceKm: linkPathDistancesM[index] / 1000 }
          : {}),
        accessTransferStreetPathVerified: linkStreetVerified[index] !== 0,
        ...(
          target.lon !== transferCoordinate[0]
          || target.lat !== transferCoordinate[1]
            ? { accessServiceCoordinate: [target.lon, target.lat] }
...
export function routeNativeCoordinateFrontier(storePath, request) {
  ...
  return {
    ...
    diagnostics: {
      ...
      queryMs: result.queryNs / 1e6,
      accessReductionMs: result.accessReductionNs / 1e6,
      snapMs: result.snapNs / 1e6,
      searchMs: result.searchNs / 1e6,
...
export function routeNativeCoordinateFrontiers(storePath, request) {
  ...
  const diagnostics = {
    ...
    queryMs: result.queryNs / 1e6,
    accessReductionMs: result.accessReductionNs / 1e6,
    originAccessReductionMs: result.originAccessReductionNs / 1e6,
    destinationAccessReductionMs: result.destinationAccessReductionNs / 1e6,
    snapMs: result.snapNs / 1e6,
    originSearchMs: result.originSearchNs / 1e6,
    destinationSearchMs: result.destinationSearchNs / 1e6,
...
function normalizeNativeStreetPathResult(result, fromCoordinate, toCoordinate) {
  ...
  return {
    ...
    nativeQueryMs: result.queryNs / 1e6,
...
export function rasterNativeStreetSurface(storePath, value) {
  ...
  const nodeEvidenceLimit = Math.max(1, Math.min(100_000, Math.floor(Number(value?.nodeEvidenceLimit) || 30_000)))
  const requestedEdgeEvidenceLimit = Number(value?.edgeEvidenceLimit)
  const edgeEvidenceLimit = requestedEdgeEvidenceLimit === 0
    ? 0
    : Math.max(1, Math.min(100_000, Math.floor(requestedEdgeEvidenceLimit) || 12_000))
  ...
  const edgeEvidenceCount = packedEdges?.count ?? (Array.isArray(result.edgeEvidence) ? result.edgeEvidence.length : 0)
  return {
    ...
    diagnostics: {
      ...
      nodeEvidenceCount: Array.isArray(result.nodeEvidence) ? result.nodeEvidence.length : 0,
      nodeEvidenceTruncated: result.nodeEvidenceTruncated === true,
      nodeEvidenceLimit,
      edgeEvidenceCount,
      edgeEvidenceTruncated: result.edgeEvidenceTruncated === true,
      edgeEvidenceLimit,
...
export function routeNativeTimedConnectors(storePath, value) {
  ...
  return {
    ...
    diagnostics: {
      ...
      aggregateCchQueryMs: result.aggregateCchQueryNs / 1e6,
      matrixCchAccelerated: result.matrixCchAccelerated === true,
      matrixCchQueryMs: result.matrixCchQueryNs / 1e6,
...
export function routeNativeDriveMatrix(accelerator, request) {
  ...
  return {
    ...
    diagnostics: {
      ...
      trafficCustomizationMs: result.trafficCustomizationNs / 1e6,
