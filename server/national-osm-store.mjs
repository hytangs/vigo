...
// contract. Rebuild from the source PBF when this contract changes.
const streetStoreSchemaVersion = 'vigo.street.store.v3'
const streetStoreTableContract = Object.freeze({
  metadata: Object.freeze(['key:TEXT:0:1:0', 'value:TEXT:1:0:0']),
  nodes: Object.freeze([
    'node_id:INTEGER:0:1:0', 'lat:REAL:1:0:0', 'lon:REAL:1:0:0',
  ]),
  walk_nodes: Object.freeze([
    'node_id:INTEGER:0:1:0', 'lat:REAL:1:0:0', 'lon:REAL:1:0:0',
  ]),
  edges: Object.freeze([
    'from_node:INTEGER:1:0:0', 'to_node:INTEGER:1:0:0',
    'distance_m:REAL:1:0:0', 'way_id:INTEGER:1:0:0',
  ]),
})
const streetStoreIndexContract = Object.freeze({
  walk_nodes_lat_lon: Object.freeze({
    table: 'walk_nodes',
    columns: Object.freeze(['lat:0:BINARY', 'lon:0:BINARY']),
  }),
  edges_from: Object.freeze({
    table: 'edges',
    columns: Object.freeze(['from_node:0:BINARY']),
  }),
})
const drivingStoreTableContract = Object.freeze({
  drive_nodes: Object.freeze([
    'node_id:INTEGER:0:1:0', 'lat:REAL:1:0:0', 'lon:REAL:1:0:0',
  ]),
  drive_edges: Object.freeze([
    'from_node:INTEGER:1:0:0', 'to_node:INTEGER:1:0:0',
    'distance_m:REAL:1:0:0', 'travel_time_s:REAL:1:0:0',
    'way_id:INTEGER:1:0:0', 'road_class:INTEGER:1:0:0',
  ]),
})
const drivingStoreIndexContract = Object.freeze({
  // The compact drive table is ordered by the from-node primary lookup only.
  // The latitude/longitude index was never used by the accelerator or the
  // service-edge query; both resolve coordinates through INTEGER PRIMARY KEY.
  drive_edges_from: Object.freeze({
    table: 'drive_edges',
    columns: Object.freeze(['from_node:0:BINARY']),
  }),
})
const streetStoreMetadataMaxRows = 128
const streetStoreMetadataMaxKeyBytes = 256
const streetStoreMetadataMaxValueBytes = 1024 * 1024
const streetStoreMetadataMaxTotalBytes = 2 * 1024 * 1024
...
const nativeStreetRouteCacheMaximumEntries = 2_048
const nativeStreetRouteCaches = new Map()
...
function driveIndexesPresent(storePath) {
  ...
  try {
    const expected = Object.keys(drivingStoreIndexContract)
...
function prepareNationalOsmDriveProfile(state, storePath, options, startedAt) {
  ...
  return {
    ...
    nativeConfigureMs: nativeDriveKernel?.configureMs ?? 0,
 ...
 */
export function forEachNationalOsmWalkNode(storePath, callback, options = {}) {
...
export function nationalOsmStoreDiagnostics(storePath) {
  ...
  return {
    ...
    queryMs: Number((accelerator?.queryMs ?? 0).toFixed(3)),
    ...
    driveAccelerator: {
      ...
      queryMs: Number((driveAccelerator?.queryMs ?? 0).toFixed(3)),
...
function invalidateStreetStore(storePath) {
  ...
  cached.streetRouteCache.clear()
  cached.drivePathCache.clear()
...
export function disposeNationalOsmStore(storePath) {
  ...
  nativeStreetRouteCaches.delete(path.resolve(storePath))
...
function validateSupplementalStoreContract(db, storePath, tableContract, indexContract) {
  const tableNames = Object.keys(tableContract)
  const indexNames = Object.keys(indexContract)
  ...
  for (const tableName of tableNames) {
    ...
    if (!object || object.type !== 'table' || object.tbl_name !== tableName) {
      throw streetStoreAdmissionError(
        ...
        `Required table "${tableName}" is missing from the driving profile.`,
      )
    }
    ...
    const expectedColumns = tableContract[tableName]
    if (
  ...
  if (indexNames.length) {
    for (const [indexName, contract] of Object.entries(indexContract)) {
      const object = schemaByName.get(indexName)
      if (!object || object.type !== 'index' || object.tbl_name !== contract.table) {
        throw streetStoreAdmissionError(
          storePath,
          'required_index_missing',
          `Required driving index "${indexName}" is missing.`,
        )
      }
      const actualColumns = db.prepare(`
      ...
      `).all(indexName).map((column) => (
        `${String(column.name)}:${Number(column.desc)}:${String(column.coll ?? '').toUpperCase()}`
      ))
      if (
        actualColumns.length !== contract.columns.length
        || actualColumns.some((column, index) => column !== contract.columns[index])
      ) {
        throw streetStoreAdmissionError(
          storePath,
...
function readStreetStoreMetadata(db, storePath) {
  try {
    const rows = db.prepare(`
      ...
      ORDER BY key
      LIMIT ?
    `).all(streetStoreMetadataMaxRows + 1)
    if (rows.length > streetStoreMetadataMaxRows) {
      throw streetStoreAdmissionError(
        storePath,
        'metadata_budget_exceeded',
        `Metadata contains more than ${streetStoreMetadataMaxRows} rows.`,
      )
    }
    let totalBytes = 0
    const entries = []
    for (const row of rows) {
      ...
      if (
        keyBytes > streetStoreMetadataMaxKeyBytes
        || valueBytes > streetStoreMetadataMaxValueBytes
        || totalBytes > streetStoreMetadataMaxTotalBytes
      ) {
        throw streetStoreAdmissionError(
          storePath,
          'metadata_budget_exceeded',
          `Metadata exceeds the ${streetStoreMetadataMaxTotalBytes}-byte bounded admission budget.`,
        )
      }
      try {
...
function admitRuntimeSnapshotStore(db, storePath, metadata, startedAt) {
  const metadataObject = db.prepare(`
    ...
    WHERE name='metadata'
  `).get()
  if (!metadataObject || metadataObject.type !== 'table' || metadataObject.tbl_name !== 'metadata') {
    throw streetStoreAdmissionError(storePath, 'required_table_missing', 'Runtime snapshot metadata table is missing.')
  }
  const metadataColumns = db.prepare(`
    SELECT name,type,"notnull" AS not_null,pk,hidden
    FROM pragma_table_xinfo('metadata')
    ORDER BY cid
  `).all().map((column) => (
    `${String(column.name)}:${String(column.type ?? '').toUpperCase()}:${Number(column.not_null)}:${Number(column.pk)}:${Number(column.hidden)}`
  ))
  const expectedColumns = streetStoreTableContract.metadata
  if (
    metadataColumns.length !== expectedColumns.length
    || metadataColumns.some((column, index) => column !== expectedColumns[index])
  ) {
    throw streetStoreAdmissionError(
      storePath,
      'table_schema_mismatch',
      `Runtime metadata columns are ${JSON.stringify(metadataColumns)}; expected ${JSON.stringify(expectedColumns)}.`,
    )
  }
  const unexpectedObjects = db.prepare(`
    SELECT type,name
    FROM sqlite_schema
    WHERE name NOT LIKE 'sqlite_%' AND name <> 'metadata'
    LIMIT 1
  `).get()
  if (unexpectedObjects) {
    throw streetStoreAdmissionError(
      storePath,
      'runtime_layout_not_compact',
      `Runtime snapshot store still contains ${unexpectedObjects.type}:${unexpectedObjects.name}.`,
    )
  }
  if (metadata.schemaVersion !== streetStoreSchemaVersion) {
    throw streetStoreAdmissionError(
  ...
  if (driveSnapshotRequired && !fsSync.existsSync(driveAcceleratorSnapshotPath(storePath))) {
    throw streetStoreAdmissionError(
      ...
      `The runtime layout requires ${driveAcceleratorSnapshotPath(storePath)} for drive/accessibility routing.`,
    )
  }
  return {
    metadata,
    admission: Object.freeze({
      ...
      metadataBudgetBytes: streetStoreMetadataMaxTotalBytes,
...
function admitSourceStreetStore(db, storePath, metadata, startedAt) {
  ...
  const tableNames = Object.keys(streetStoreTableContract).filter((tableName) => (
    tableName !== 'nodes' || !compactStorageLayout
  ))
  const indexNames = Object.keys(streetStoreIndexContract)
  const objectNames = [...tableNames, ...indexNames]
  const schemaRows = db.prepare(`
    SELECT type, name, tbl_name, sql
    FROM sqlite_schema
  ...
  for (const tableName of tableNames) {
    const object = schemaByName.get(tableName)
    if (!object) {
      throw streetStoreAdmissionError(storePath, 'required_table_missing', `Required table "${tableName}" is missing.`)
    }
  ...
  const tableRows = db.prepare(`
    WITH required(name) AS (VALUES ${tableNames.map(() => '(?)').join(',')})
  ...
  `).all(...tableNames)
  const columnsByTable = new Map(tableNames.map((name) => [name, []]))
  for (const column of tableRows) {
    columnsByTable.get(String(column.table_name))?.push(
      `${String(column.name)}:${String(column.type ?? '').toUpperCase()}:${Number(column.not_null)}:${Number(column.pk)}:${Number(column.hidden)}`,
    )
  }
  for (const [tableName, expectedColumns] of Object.entries(streetStoreTableContract)) {
  ...
  for (const [indexName, contract] of Object.entries(streetStoreIndexContract)) {
    const object = schemaByName.get(indexName)
    if (!object) {
      throw streetStoreAdmissionError(storePath, 'required_index_missing', `Required index "${indexName}" is missing.`)
    }
    const normalizedSql = String(object.sql ?? '').replace(/\s+/g, '').toLowerCase()
    if (
      object.type !== 'index'
      || object.tbl_name !== contract.table
      || !normalizedSql.startsWith('createindex')
      || normalizedSql.includes('where')
    ) {
      throw streetStoreAdmissionError(
        storePath,
        'index_definition_mismatch',
        `Required index "${indexName}" does not match index:${contract.table}.`,
      )
    }
  }
  const indexRows = db.prepare(`
    WITH required(name) AS (VALUES ${indexNames.map(() => '(?)').join(',')})
  ...
  `).all(...indexNames)
  const columnsByIndex = new Map(indexNames.map((name) => [name, []]))
  for (const column of indexRows) {
  ...
  validateSupplementalStoreContract(
    db,
    storePath,
    drivingStoreTableContract,
    deferredDriveIndexes ? {} : drivingStoreIndexContract,
  )
  if (metadata.sourceModel !== 'pbf') {
    throw streetStoreAdmissionError(
      storePath,
  ...
  return {
    ...
    admission: Object.freeze({
      ...
      schemaVersion: metadata.schemaVersion,
      sourceModel: 'pbf',
      requiredTableCount: tableNames.length + Object.keys(drivingStoreTableContract).length,
      requiredIndexCount: indexNames.length + (
        deferredDriveIndexes ? 0 : Object.keys(drivingStoreIndexContract).length
      ),
      metadataRows: Object.keys(metadata).length,
      metadataBudgetBytes: streetStoreMetadataMaxTotalBytes,
      admissionMs: Number((performance.now() - startedAt).toFixed(3)),
...
function openStreetStoreWithAdmission(storePath, admissionFunction, requiredLayout = 'diagnostic') {
  ...
  try {
    ...
    const state = {
      ...
      runtimeSnapshotOnly,
      drivePathCache: new Map(),
      streetRouteCache: new Map(),
      accelerator: null,
...
function streetPathCacheKey(fromCoordinate, toCoordinate, maxDistanceKm) {
...
function nativeDrivePathBetween(state, fromCoordinate, toCoordinate, maxDistanceKm, origins, targets, traffic) {
  ...
  const searchMs = result.queryMs
  accelerator.queryCount += 1
  accelerator.queryMs += searchMs
  ...
  return {
    ...
    trafficCustomizationMs: Number((result.trafficCustomizationNs / 1e6).toFixed(3)),
    trafficMetricReused: result.trafficMetricReused === true,
    fastPathMs: Number((result.fastPathQueryNs / 1e6).toFixed(3)),
    distancePathMs: Number((result.distancePathQueryNs / 1e6).toFixed(3)),
    fallbackMs: Number((result.fallbackQueryNs / 1e6).toFixed(3)),
...
function drivePathBetween(state, fromCoordinate, toCoordinate, maxDistanceKm, traffic) {
  const cacheKey = `${streetPathCacheKey(fromCoordinate, toCoordinate, maxDistanceKm)}:${traffic?.cacheKey ?? 'free-flow'}`
  if (state.drivePathCache.has(cacheKey)) {
    return { result: state.drivePathCache.get(cacheKey), cacheHit: true }
  }
  const startedAt = performance.now()
  ...
  if (state.drivePathCache.size >= 20_000) {
    state.drivePathCache.delete(state.drivePathCache.keys().next().value)
  }
  state.drivePathCache.set(cacheKey, result)
  return { result, cacheHit: false }
}

function streetRouteCacheKey(request) {
  return JSON.stringify([
    request.mode,
    request.origin?.coordinate,
    request.destination?.coordinate,
    request.departMinutes,
    request.walkingSpeedKph,
    request.maxStreetKm,
    request.trafficCacheKey,
  ])
}

function streetRouteSearchDiagnostics(queryMs, search, cacheHit, mode) {
  const effectiveCacheHit = cacheHit || search?.cacheHit === true
  return {
    queryMs: Number(queryMs.toFixed(3)),
    searchMs: effectiveCacheHit ? 0 : Number((search?.searchMs ?? queryMs).toFixed(3)),
    ...
    trafficCustomizationMs: search?.trafficCustomizationMs,
    trafficMetricReused: search?.trafficMetricReused === true,
    fastPathMs: search?.fastPathMs,
    distancePathMs: search?.distancePathMs,
    fallbackMs: search?.fallbackMs,
...
export function routeNationalStreetStore(storePath, request) {
  ...
  const departMinutes = Number(request?.departMinutes)
  const normalizedRequest = {
    ...request,
    mode,
    departMinutes: Number.isFinite(departMinutes) ? departMinutes : 8 * 60,
  }
  if (
  ...
  if (mode === 'walk' && !nativeWalk) {
    return blockedStreetRoute(
      ...
      {
        searchStats: streetRouteSearchDiagnostics(performance.now() - startedAt, null, false, mode),
      },
    )
  }
  ...
  const routeCacheKey = streetRouteCacheKey(normalizedRequest)
  let routeCache
  if (nativeWalk) {
    const resolvedStorePath = path.resolve(storePath)
    routeCache = nativeStreetRouteCaches.get(resolvedStorePath)
    if (!routeCache) {
      routeCache = new Map()
      nativeStreetRouteCaches.set(resolvedStorePath, routeCache)
      while (nativeStreetRouteCaches.size > streetStoreCacheMaximumEntries) {
        nativeStreetRouteCaches.delete(nativeStreetRouteCaches.keys().next().value)
      }
    }
  } else {
    routeCache = state.streetRouteCache
  }
  const cached = routeCache.get(routeCacheKey)
  if (cached) {
    const { _search, ...cachedPlan } = cached
    return {
      ...cachedPlan,
      diagnostics: {
        ...cachedPlan.diagnostics,
        searchStats: streetRouteSearchDiagnostics(performance.now() - startedAt, _search, true, mode),
      },
    }
  }
  ...
  let search
  if (mode === 'drive') {
    const drive = drivePathBetween(state, originCoordinate, destinationCoordinate, maxStreetKm, traffic)
    pathResult = drive.result.path
    search = { ...drive.result, cacheHit: drive.cacheHit }
  } else if (nativeWalk) {
    ...
    search = {
      searchMs: pathResult?.nativeQueryMs ?? performance.now() - startedAt,
  ...
  if (!pathResult) {
    ...
    return blockedStreetRoute(
      ...
      {
        searchStats: streetRouteSearchDiagnostics(performance.now() - startedAt, search, false, mode),
        ...(mode === 'drive' ? {
          scheduleMode: 'none',
  ...
  const cachedPlan = { ...plan, _search: search }
  const maximumCacheEntries = nativeWalk
    ? nativeStreetRouteCacheMaximumEntries
    : 20_000
  if (routeCache.size >= maximumCacheEntries) {
    routeCache.delete(routeCache.keys().next().value)
  }
  routeCache.set(routeCacheKey, cachedPlan)
  return plan
}
...
export function routeNationalStreetMatrix(storePath, request = {}, options = {}) {
  ...
  for (let originIndex = 0; originIndex < origins.length; originIndex += 1) {
    for (let destinationIndex = 0; destinationIndex < destinations.length; destinationIndex += 1) {
      ...
      rows.push({
        originIndex,
        destinationIndex,
        status: Number.isFinite(distanceM) && Number.isFinite(durationMinutes) ? 'ready' : 'blocked',
        distanceKm: Number.isFinite(distanceM) ? distanceM / 1_000 : null,
  ...
  return {
    ...
    diagnostics: {
      ...
      nativeQueryMs: Number(native.queryMs.toFixed(3)),
      nodeApiWallMs: Number(native.nodeApiWallMs.toFixed(3)),
...
export function streetNetworkTimedConnectors(storePath, value, options = {}) {
  ...
  options.onProgress?.({
    phase: 'accessibility-connectors',
    progress: 0.01,
