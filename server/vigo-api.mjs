...
import { analyzeScenarioSurface } from './scenario-analysis.mjs'
...
const workspaceMaintenanceProjects = new Set()
...
const accessibilityAnalysisCancellationGraceMs = Math.max(
  1_000,
  Math.min(60_000, Number(process.env.VIGO_ACCESSIBILITY_CANCEL_GRACE_MS) || 30_000),
)
...
const nationalRouteResponseCacheMaxEntries = Math.max(1, Math.floor(Number(process.env.VIGO_ROUTE_RESPONSE_CACHE_MAX_ENTRIES) || 128))
const nationalRouteResponseCacheMaxBytes = Math.max(1024 * 1024, Math.floor(Number(process.env.VIGO_ROUTE_RESPONSE_CACHE_MAX_BYTES) || 8 * 1024 * 1024))
const nationalRouteResponseAlgorithmGeneration = 'vigo.routing.http-response-cache.v1'
const preSerializedJsonResponses = new WeakMap()

class NationalRouteResponseCache {
  ...
  get(key) {
    ...
    // Worker results are immutable after dispatch. Retaining the owned object
    // avoids a second structured clone before the HTTP layer serializes it.
    return entry.value
  }
  ...
  set(key, value, projectId) {
    ...
    if (stored && typeof stored === 'object') preSerializedJsonResponses.set(stored, serialized)
    this.#delete(key, false)
    if (bytes > this.maxBytes) {
      this.rejections += 1
      return false
    }
    while (this.values.size >= this.maxEntries || this.estimatedBytes + bytes > this.maxBytes) {
      const oldestKey = this.values.keys().next().value
      if (oldestKey === undefined) break
      this.#delete(oldestKey, false)
      this.evictions += 1
    }
    this.values.set(key, { value: stored, bytes, projectId })
  ...
  #delete(key, countInvalidation) {
    const entry = this.values.get(key)
    if (!entry) return false
    this.values.delete(key)
    this.estimatedBytes -= entry.bytes
    if (countInvalidation) this.invalidations += 1
    return true
  }
}

const nationalRouteResponseCache = new NationalRouteResponseCache({
  maxEntries: nationalRouteResponseCacheMaxEntries,
  maxBytes: nationalRouteResponseCacheMaxBytes,
})
...
function stableRouteCacheValue(value) {
  if (Array.isArray(value)) return value.map(stableRouteCacheValue)
  if (!value || typeof value !== 'object') return value
  return Object.fromEntries(
    Object.keys(value)
      .filter((key) => value[key] !== undefined)
      .sort()
      .map((key) => [key, stableRouteCacheValue(value[key])]),
  )
}
...
function nationalRoutingPrewarmContext(context = {}) {
  ...
  const streetContextKey = streetStorePath
    ? crypto.createHash('sha1').update(path.resolve(streetStorePath)).digest('hex').slice(0, 10)
    : 'none'
  if (!serviceDate) {
    return {
  ...
  const serviceDay = ['saturday', 'sunday'].includes(context.serviceDay) ? context.serviceDay : 'weekday'
...
function cacheableNationalRouteResponse(responseResult) {
  const plan = responseResult?.plan
  return Boolean(
    plan?.status === 'ready'
    || (
      plan?.status === 'blocked'
      && plan?.diagnostics?.failure?.retryable === false
    ),
  )
}
...
class NationalRouteWorkerClient {
  ...
  enqueue(operation, request, signal, onProgress) {
    ...
    return new Promise((resolve, reject) => {
      const job = {
        ...
        cancelTimer: null,
        cancelled: false,
        cancellation: ['accessibility-range', 'street-surface', 'street-matrix', 'street-route-batch'].includes(operation)
  ...
  diagnostics() {
    ...
    return {
      storeKey: crypto.createHash('sha1').update(this.storePath).digest('hex').slice(0, 10),
      ...
      accessibilityCancellationGraceMs: accessibilityAnalysisCancellationGraceMs,
  ...
  #handleMessage(message, generation) {
    ...
    if (message?.type === 'complete') {
      ...
      if (job.operation === 'prepare') {
        ...
        if (job.request?.streetStorePath && message.result?.streetStore?.ready) {
          this.streetPrepareResult = message.result
          this.streetPrepareStorePath = path.resolve(String(job.request.streetStorePath))
        }
        if (job.request?.streetStorePath && message.result?.osmStopTransfers?.ready) {
          this.transferPrepareResult = message.result
          this.transferPrepareStorePath = path.resolve(String(job.request.streetStorePath))
        }
      }
  ...
  #cancel(job) {
    ...
    if (!['prepare', 'prepare-derived', 'prepare-routing-access'].includes(job.operation)) {
      const cancellationGraceMs = ['accessibility-range', 'street-surface', 'street-matrix'].includes(job.operation)
        ? accessibilityAnalysisCancellationGraceMs
...
class NationalRouteWorkerPool {
  ...
  snapshot() {
    ...
    return {
      ...
      responseCache: nationalRouteResponseCache.snapshot(),
...
function normalizeAutomaticCacheCleanup(value) {
  return value !== false
}
...
async function loadRuntimeConfig() {
  ...
  try {
    ...
    runtimeConfig = {
      ...
      automaticCacheCleanup: normalizeAutomaticCacheCleanup(stored.automaticCacheCleanup),
    }
...
async function configStatus() {
  return {
    ...
    automaticCacheCleanup: normalizeAutomaticCacheCleanup(runtimeConfig.automaticCacheCleanup),
...
async function updateRuntimeConfig(body = {}) {
  ...
  if (requestedStorageRoot !== storageRoot) {
    ...
    nationalRouteResponseCache.clear()
  ...
  runtimeConfig = {
    ...
    automaticCacheCleanup: normalizeAutomaticCacheCleanup(
      body.automaticCacheCleanup ?? runtimeConfig.automaticCacheCleanup,
    ),
...
function routingStoreSelection(projectId, project, requestedFeedId) {
  ...
  return {
    ...
    // otherwise a multi-feed Evidence comparison could silently analyze the
...
function currentNationalRoutingServiceContext(date = new Date()) {
  ...
  return {
    serviceDate,
    serviceDay: weekday === 0 ? 'sunday' : weekday === 6 ? 'saturday' : 'weekday',
...
async function readLocalStreetGeometry(projectId, url) {
  ...
  if (project.osmStreetIndex?.status !== 'ready' || !await exists(streetStoreFile(projectId))) {
    const error = new Error('This workspace has no ready local OSM street index. Import an .osm.pbf file first.')
  ...
  return {
    ...
    osmStreetIndex: street
      ? {
        ...
        sourceSha256: street.sourceSha256,
...
async function analyzeGtfsRoute(projectId, body = {}) {
  ...
  const cacheKey = [context.storePath, storeStats.size, storeStats.mtimeMs, context.sourceScope, routeId].join(':')
  let analysis = gtfsRouteAnalysisCache.get(cacheKey)
  if (!analysis) {
    analysis = readGtfsRouteAnalysis(context.storePath, routeId, { sourceScope: context.sourceScope })
    if (!analysis) {
...
async function listProjects() {
  if (await setupRequired()) return listProjectsRaw({ hydrate: false })
...
async function updateProject(projectId, body) {
  ...
  nationalRouteResponseCache.invalidateProject(projectId)
  return updated
}

async function deleteProject(projectId) {
  if (workspaceMaintenanceProjects.has(projectId)) {
    const error = new Error('This workspace is already being modified.')
    error.statusCode = 409
    throw error
  }
  if (nationalImportProjects.has(projectId)) {
    const error = new Error('Wait for the active GTFS or OSM import to finish before removing this workspace.')
    error.statusCode = 409
    throw error
  }

  workspaceMaintenanceProjects.add(projectId)
  try {
    const project = await readProjectMetadata(projectId)
    ...
    workspaceMaintenanceProjects.delete(projectId)
  }
}
...
function applicationMemoryCacheSnapshot() {
  const routeResponses = nationalRouteResponseCache.snapshot()
  const entries = routeResponses.entries
    + nationalRoutingServiceCoverageCache.size
    + nationalPreviewRefreshes.size
    + gtfsRouteAnalysisCache.size
    + routingStoreSourceScopesCache.size
    + scenarioRouteStopCache.size
  return {
    entries,
    estimatedBytes: routeResponses.estimatedBytes,
    routeResponses,
    serviceCoverageEntries: nationalRoutingServiceCoverageCache.size,
    previewEntries: nationalPreviewRefreshes.size,
    gtfsAnalysisEntries: gtfsRouteAnalysisCache.size,
    sourceScopeEntries: routingStoreSourceScopesCache.size,
    scenarioStopEntries: scenarioRouteStopCache.size,
  }
}

async function applicationStagingCacheTargets() {
  const entries = await fs.readdir(storageRoot, { withFileTypes: true }).catch((error) => {
    if (error?.code === 'ENOENT') return []
    throw error
  })
  const targets = []
  const skippedActiveProjectIds = []

  for (const entry of entries) {
    if (!entry.isDirectory()) continue
    if (nationalImportProjects.has(entry.name)) {
      skippedActiveProjectIds.push(entry.name)
      continue
    }
    const targetPath = path.join(storageRoot, entry.name, '.vigo', 'staging')
    const measure = await measureStoragePath(targetPath)
    if (measure.fileCount > 0) targets.push({ projectId: entry.name, path: targetPath, ...measure })
  }

  return { targets, skippedActiveProjectIds }
}

async function inspectApplicationCache() {
  const { targets, skippedActiveProjectIds } = await applicationStagingCacheTargets()
  const staging = addStorageMeasures(...targets)
  const memory = applicationMemoryCacheSnapshot()
  return {
    schemaVersion: 'vigo.cache_maintenance.preview.v1',
    automatic: normalizeAutomaticCacheCleanup(runtimeConfig.automaticCacheCleanup),
    estimatedFreedBytes: staging.bytes + memory.estimatedBytes,
...
async function cleanApplicationCache({ reason = 'manual' } = {}) {
  const before = await inspectApplicationCache()
  const { targets, skippedActiveProjectIds } = await applicationStagingCacheTargets()

  await Promise.all(targets.map((target) => fs.rm(target.path, { recursive: true, force: true })))
  nationalRouteResponseCache.clear()
  nationalRoutingServiceCoverageCache.clear()
  nationalPreviewRefreshes.clear()
  gtfsRouteAnalysisCache.clear()
  routingStoreSourceScopesCache.clear()
  scenarioRouteStopCache.clear()

  const after = await inspectApplicationCache()
  return {
    schemaVersion: 'vigo.cache_maintenance.result.v1',
    reason,
    cleanedAt: now(),
    freedBytes: Math.max(0, before.estimatedFreedBytes - after.estimatedFreedBytes),
    removedFileCount: Math.max(0, before.fileCount - after.fileCount),
    removedMemoryEntries: Math.max(0, before.memory.entries - after.memory.entries),
    skippedActiveProjectIds,
    cache: after,
  }
}

let automaticCacheCleanupCompleted = false

async function runAutomaticCacheCleanup() {
  if (automaticCacheCleanupCompleted) return null
  automaticCacheCleanupCompleted = true
  if (!normalizeAutomaticCacheCleanup(runtimeConfig.automaticCacheCleanup)) return null
  return cleanApplicationCache({ reason: 'automatic-startup' })
}
...
async function inspectWorkspaceCleanup(projectId) {
  const project = await readProjectMetadata(projectId)
...
async function deepCleanWorkspace(projectId, body = {}) {
  if (workspaceMaintenanceProjects.has(projectId)) {
    const error = new Error('This workspace is already being cleaned.')
    error.statusCode = 409
    throw error
  }
  workspaceMaintenanceProjects.add(projectId)
  ...
  try {
    ...
    if (body.confirmation !== project.name) {
      const error = new Error('Type the exact workspace name to confirm deep cleanup.')
      error.statusCode = 400
      throw error
    }
    if (nationalImportProjects.has(projectId)) {
      const error = new Error('Wait for the active GTFS or OSM import to finish before cleaning this workspace.')
      error.statusCode = 409
      throw error
    }

    const before = await inspectWorkspaceCleanup(projectId)
    ...
    await markStorageInitialized('deep-clean-workspace')
    invalidateProjectRuntimeCaches(projectId)
    const cleanedProject = await readProject(projectId)
    const after = await inspectWorkspaceCleanup(projectId)
    return {
      ok: true,
      project: cleanedProject,
      workspace: after,
      cleanup: {
        schemaVersion: 'vigo.workspace_cleanup.result.v1',
        cleanedAt: now(),
...
async function commitIndexedFeed(projectId, body, options = {}) {
  ...
  const updated = await writeProject({
    ...
    ...(options.promoteToProjectStore === true ? {
      ...
      osmStreetIndex: project.osmStreetIndex
        ? { ...project.osmStreetIndex, datasetFingerprint: undefined }
...
async function startNationalGtfsImport(projectId, body) {
  ...
  const cleanupUploadedSource = async () => {
    if (!cleanupSource || sourceRemoved) return
    sourceRemoved = true
  ...
  try {
    await persistNationalJob(projectId, job)
    worker = new Worker(new URL('./national-gtfs-worker.mjs', import.meta.url), {
      workerData: { zipPath: sourcePath, outputPath },
    })
  } catch (error) {
...
async function startNationalOsmImport(projectId, body) {
  ...
  try {
    await persistNationalJob(projectId, job)
    worker = new Worker(new URL('./national-osm-worker.mjs', import.meta.url), {
      workerData: {
        pbfPath: sourcePath,
        outputPath,
      ...
      if (message?.type === 'progress') {
        ...
        const project = await enqueueProjectWrite(projectId, async () => {
          ...
          return writeProject({ ...current, osmStreetIndex: {
            ...
            sourceSha256: message.result.sourceFingerprint,
...
async function cancelNationalImportJob(projectId, jobId) {
  ...
  const error = new Error('This preparation job is no longer attached to a live worker. Reload the workspace and retry it.')
  error.statusCode = 409
...
async function runSingleNationalRoute(projectId, body, signal, options = {}) {
  ...
  const storePath = selection.storePath
  ...
  const pedestrianAccessRequested = mode === 'walk' || (mode === 'transit' && !exactStopEndpoints)
  ...
  const transitStreetPath = mode === 'transit' && exactStopEndpoints
    ? undefined
    : streetPath
  const windowMinutes = mode !== 'transit' || body?.timePreference === 'arrive'
    ? 0
    : Math.max(0, Math.min(30, Math.round(Number(body?.departureWindowMinutes ?? 0))))
  const centerMinutes = Number(body?.departMinutes ?? 8 * 60)
  // Interactive transit defaults to the same explicit generalized objective
  // as the desktop: exact timetable routing first, then a transparent choice
  // between the earliest witness and the minimum-boarding witness inside the
  // declared arrival window. Advanced callers can still request
  // strict earliest arrival with routingPreference="fastest".
  const routingPreference = resolveNationalInteractiveRoutingPreference(body?.routingPreference)
  const requireTransitRide = mode === 'transit' && options.requireTransitRide === true
  const realtimeRoutingRequested = mode === 'transit'
    && Array.isArray(body?.realtimeSnapshot?.tripUpdates)
    && body.realtimeSnapshot.tripUpdates.length > 0
  const trafficRoutingRequested = mode === 'drive'
    && body?.trafficSnapshot
    && typeof body.trafficSnapshot === 'object'
    && !Array.isArray(body.trafficSnapshot)
  ...
  const routeRequest = windowMinutes
    ? {
      ...
      // The HTTP layer owns the serialized semantic-response cache. Avoid
      // retaining a second geometry-rich copy inside the worker isolate.
      __disableResultCache: true,
      }
    : {
        ...publicRoutingBody,
  ...
  const responseCacheKey = realtimeRoutingRequested || trafficRoutingRequested
    ? null
    : await nationalRouteResponseCacheKey(storePath, transitStreetPath, routeRequest)
  const retained = responseCacheKey ? nationalRouteResponseCache.get(responseCacheKey) : undefined
  if (retained && (
    body?.includeEarliestTransit !== true
    || retained.earliestTransit
  )) return retained
  ...
  if (mode !== 'transit') {
    ...
    const responseResult = { plan, choices: [plan] }
    if (!signal?.aborted && responseCacheKey && cacheableNationalRouteResponse(responseResult)) {
      nationalRouteResponseCache.set(responseCacheKey, responseResult, projectId)
    }
...
const nationalServiceDays = new Set(['weekday', 'saturday', 'sunday'])
...
async function nationalBuildPreloadContext(storePath, requestedServiceDate, requestedServiceDay) {
  ...
  const requestedDay = nationalServiceDays.has(String(requestedServiceDay ?? '').trim())
    ? String(requestedServiceDay).trim()
    : nationalServiceDayForDate(requestedDate)
...
async function prepareNationalRouting(projectId, body, signal) {
  ...
  const routingContext = {
    serviceDate: body?.serviceDate || defaultContext.serviceDate,
    serviceDay: body?.serviceDay || defaultContext.serviceDay,
...
function scenarioStopsForStoredRoute(storePath, storeIdentity, routeId) {
  ...
  const identity = crypto.createHash('sha256')
    .update(JSON.stringify(storeIdentity))
    .digest('hex')
    .slice(0, 20)
...
async function runScenarioAnalysis(projectId, body, signal, onProgress, onPreliminary) {
  const project = await readProjectMetadata(projectId)
  const feedId = String(body?.feedId ?? '')
  const selection = routingStoreSelection(projectId, project, feedId)
  if (!selection.storePath) {
  ...
  const streetPath = project.osmStreetIndex?.status === 'ready' && await exists(streetStoreFile(projectId))
    ...
    : null
  if (!streetPath) {
    const error = new Error('Accessibility analysis requires a ready OSM pedestrian street index.')
    error.statusCode = 409
    throw error
  }
  ...
  const hydratedBody = hydrateScenarioRouteServices(storePath, storeIdentity, body)
  ...
  const mapAnalysisProgress = (stage, progress) => {
    const [start, end] = progressWindow(stage)
    const ratio = Math.max(0, Math.min(1, Number(progress ?? 0)))
    return start + ratio * (end - start)
  }
  return analyzeScenarioSurface({
    ...hydratedBody,
    feedId,
    baselineIdentity,
  }, {
    runAccessibilityRange: (rangeRequest) => nationalRouteWorkerPool.dispatch(
      storePath,
      'accessibility-range',
      {
        ...rangeRequest,
      ...
      (progress) => onProgress?.({
        ...
        progress: mapAnalysisProgress(rangeRequest.analysisStage, progress.progress),
      }),
    ),
    buildPreliminaryStreetRaster: (surfaceRequest) => nationalRouteWorkerPool.dispatch(
      ...
      (progress) => onProgress?.({
        ...progress,
        progress: mapAnalysisProgress(surfaceRequest.analysisStage, progress.progress),
      }),
    ),
    onProgress,
    onPreliminary,
...
async function runServiceEdgeDecomposition(projectId, body, signal) {
  ...
  if (!baselineFeed || !comparisonFeed) {
    const error = new Error('Both selected GTFS feeds must belong to this workspace.')
    error.statusCode = 400
    throw error
  }
...
async function setRoutingResidency(projectId, body) {
  ...
  if (body?.resident === true && String(body?.serviceDate ?? '').trim()) {
    ...
    if (streetPath) {
      try {
        accessibilityPrewarm = await nationalRouteWorkerPool.prepareRoutingAccess(
          storePath,
          {
            reason: 'accessibility-residency',
            context: {
...
async function createArtifact(projectId, body) {
  const project = await readProject(projectId)
  const artifact = {
    id: makeId('artifact'),
    schemaVersion: artifactSchemaVersion,
    type: body.type ?? 'analysis-state',
    title: body.title ?? 'Untitled artifact',
    createdAt: now(),
    sourceFeedIds: Array.isArray(body.sourceFeedIds) ? body.sourceFeedIds : [],
  }
...
async function createJob(projectId, body) {
  const project = await readProject(projectId)
  const createdAt = now()
  const job = {
    id: makeId('job'),
    kind: body.kind ?? 'analysis',
    label: body.label ?? 'Analysis job',
    status: 'complete',
    progress: 100,
    createdAt,
    finishedAt: createdAt,
    output: body.output ?? 'Completed and stored as a deterministic local job.',
  }

  await writeJson(path.join(projectMetaDir(projectId), 'jobs', `${job.id}.json`), job)
...
function sendJson(response, statusCode, value) {
  ...
  const serialized = value && typeof value === 'object'
    ? preSerializedJsonResponses.get(value) ?? JSON.stringify(value)
...
async function route(request, response) {
  ...
  if (request.method === 'GET' && pathname === '/api/cache-maintenance') {
    sendJson(response, 200, { cache: await inspectApplicationCache() })
    return true
  }

  if (request.method === 'POST' && pathname === '/api/cache-maintenance') {
    sendJson(response, 200, await cleanApplicationCache({ reason: 'manual' }))
    return true
  }
  ...
  if (projectMatch) {
    ...
    if (workspaceMaintenanceProjects.has(projectId)) {
      const error = new Error('This workspace is temporarily unavailable while deep cleanup finishes.')
      error.statusCode = 409
      throw error
    }
    ...
    if (request.method === 'GET' && action === 'workspace-cleanup') {
      sendJson(response, 200, { workspace: await inspectWorkspaceCleanup(projectId) })
      return true
    }
    ...
    if (request.method === 'POST' && action === 'workspace-cleanup') {
      sendJson(response, 200, await deepCleanWorkspace(projectId, await readBody(request)))
      return true
    }
    ...
    if (request.method === 'POST' && action === 'scenario-analysis') {
      await withRequestAbort(request, response, async (signal) => {
        const body = await readBody(request)
        const streamsProgress = String(request.headers.accept ?? '').includes('application/x-ndjson')
        if (!streamsProgress) {
          const analysis = await runScenarioAnalysis(projectId, body, signal)
          if (signal.aborted) throw makeAbortError()
        ...
        try {
          const analysis = await runScenarioAnalysis(
            ...
            (preliminary) => write({ type: 'preliminary', analysis: preliminary }),
          )
          if (signal.aborted) return
          write({ type: 'complete', analysis })
          response.end()
        } catch (error) {
    ...
    if (
      request.method === 'POST'
      && (action === 'routing-residency' || action === 'scenario-analysis-residency')
    ) {
      sendJson(response, 200, {
    ...
    if (request.method === 'POST' && action === 'artifacts') {
      sendJson(response, 201, { project: await createArtifact(projectId, await readBody(request)) })
      return true
    }

    if (request.method === 'POST' && action === 'jobs') {
      sendJson(response, 201, { project: await createJob(projectId, await readBody(request)) })
      return true
    }
  }
...
function startInMemoryHttpTransport() {
  if (typeof process.send !== 'function') {
    throw new Error('The in-memory HTTP transport requires a Node child-process IPC channel.')
  }
  ...
  process.on('message', (message) => {
    ...
    const body = typeof message.bodyBase64 === 'string'
...
if (apiTransport === 'memory') {
  await ensureStorage()
  await runAutomaticCacheCleanup().catch((error) => {
    console.warn(`VIGO automatic cache cleanup skipped: ${error instanceof Error ? error.message : String(error)}`)
  })
  process.title = 'VIGO'
