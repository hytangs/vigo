...
const supportedRasterSizes = new Set([48, 64, 96, 128, 192, 256, 384, 512, 1024])
...
const cacheLimits = Object.freeze({
  maxEntries: 12,
...
function badRequest(message, code = 'invalid_scenario_analysis_request') {
...
async function preliminaryWalkSurface(
  ...
  const networkRaster = await buildStreetRaster({
    analysisStage: 'preliminary-surface',
  ...
  ) throw new Error('Scenario analysis received an invalid preliminary OSM street-raster response.')
  const raster = {
  ...
  return {
    ...
    surface: {
      reachability: {
        ...
        cutoffMinutes: maximumCutoff,
        baselineTerminalWalkKm: request.maxWalkKm,
        scenarioTerminalWalkKm: request.scenario.policy.maxWalkKm,
        baselineTerminalWalkMinutes: Number(terminalWalkMinutes.toFixed(3)),
        scenarioTerminalWalkMinutes: Number((request.scenario.policy.maxWalkKm / request.scenario.policy.walkSpeedKph * 60).toFixed(3)),
      },
      raster: {
        width: request.rasterSize,
        height: request.rasterSize,
    ...
    diagnostics: {
      ...
      analysisEngine: 'walk_preliminary',
      accessibility: null,
      scenarioAccessibility: null,
      originAccess: {
...
function scenarioEvidenceLimitations(request, hasScenarioChanges) {
  const limitations = [
    {
...
function requireStreetEdgeBundle(edges, label) {
  if (
    ...
    throw new Error(`Scenario analysis received invalid ${label} packed street-edge evidence.`)
  }
  return edges
}
...
function cacheResult(key, result) {
  const weight = Buffer.byteLength(JSON.stringify(result))
  if (weight > cacheLimits.maxBytes) return { stored: false, weightBytes: weight }
  if (analysisCache.has(key)) {
    analysisCacheBytes -= analysisCache.get(key).weightBytes
    analysisCache.delete(key)
  }
  while (
    analysisCache.size
    && (analysisCache.size >= cacheLimits.maxEntries || analysisCacheBytes + weight > cacheLimits.maxBytes)
  ) {
    const oldestKey = analysisCache.keys().next().value
    const oldest = analysisCache.get(oldestKey)
    analysisCache.delete(oldestKey)
    analysisCacheBytes -= oldest.weightBytes
  }
  analysisCache.set(key, { result: clone(result), weightBytes: weight })
  analysisCacheBytes += weight
  return { stored: true, weightBytes: weight }
}

export function clearScenarioAnalysisCache() {
  analysisCache.clear()
  analysisCacheBytes = 0
}

export function scenarioAnalysisCacheSnapshot() {
  return {
    entries: analysisCache.size,
    bytes: analysisCacheBytes,
    maxEntries: cacheLimits.maxEntries,
    maxBytes: cacheLimits.maxBytes,
  }
}

export function validateScenarioAnalysisRequest(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw badRequest('Scenario analysis requires a JSON object.')
  }
  ...
  if (!supportedRasterSizes.has(rasterSize)) {
    throw badRequest('rasterSize must be one of 48, 64, 96, 128, 192, 256, 384, 512, or 1024.')
  }
  ...
  const scenarioValue = value.scenario && typeof value.scenario === 'object' && !Array.isArray(value.scenario)
    ? value.scenario
    : {}
  const services = Array.isArray(scenarioValue.services)
    ? value.scenario.services.map(normalizedService)
    : []
  ...
  const { stops } = uniqueScenarioStops(services)
  if (stops.length > maximumScenarioStops) {
    throw badRequest(`Scenario analysis is limited to ${maximumScenarioStops} unique scenario stops.`)
  }
  ...
  const excludedPatternIds = (Array.isArray(scenarioValue.excludedPatternIds)
    ...
    : []).flatMap((value, index) => {
      ...
      const routeId = compactText(value.routeId, '', 240)
      const patternId = compactText(value.patternId, '', 320)
  ...
  const policyValue = scenarioValue.policy
    && typeof scenarioValue.policy === 'object'
    && !Array.isArray(scenarioValue.policy)
    ? scenarioValue.policy
    : {}
  ...
  return {
    ...
    feedId: compactText(value.feedId, '__project__', 160),
    origin,
    departMinutes: boundedNumber(value.departMinutes ?? 8 * 60, 'departMinutes', 0, 2_880),
    serviceDate: compactText(value.serviceDate, '', 16),
    serviceDay: compactText(value.serviceDay, 'weekday', 16),
    ...
    scenario: {
      ...
      excludedPatternIds,
      policy: {
        maxWalkKm: Object.hasOwn(policyValue, 'maxWalkKm')
          ? boundedNumber(policyValue.maxWalkKm, 'scenario.policy.maxWalkKm', 0.2, 5)
          : baselineMaxWalkKm,
        walkSpeedKph: Object.hasOwn(policyValue, 'walkSpeedKph')
          ? boundedNumber(policyValue.walkSpeedKph, 'scenario.policy.walkSpeedKph', 1, 8)
          : baselineWalkSpeedKph,
      },
    },
  }
}

export async function analyzeScenarioSurface(
  value,
  {
    runAccessibilityRange,
    buildPreliminaryStreetRaster,
    onProgress,
    onPreliminary,
  } = {},
) {
  if (typeof runAccessibilityRange !== 'function') {
    throw new TypeError('analyzeScenarioSurface requires the unified Accessibility range function.')
  }
  const startedAt = performance.now()
  const request = validateScenarioAnalysisRequest(value)
  ...
  const retained = analysisCache.get(key)
  if (retained) {
    analysisCache.delete(key)
    analysisCache.set(key, retained)
    const result = clone(retained.result)
    result.diagnostics = {
      ...result.diagnostics,
      accessibilityDispatches: 0,
  ...
  const accessibilityRange = await runAccessibilityRange({
    analysisStage: 'baseline-range',
  ...
  if (
    !accessibilityRange
    || accessibilityRange.schemaVersion !== 'vigo.analysis.accessibility-range.v1'
    || !Array.isArray(accessibilityRange.stops)
  ) throw new Error('Scenario analysis received an invalid unified Accessibility range response.')
  transitStops = accessibilityRange.stops
  ...
  stopSelection = { diagnostics: accessibilityRange.diagnostics?.stopSelection }
  let scenarioAccessibilityRange = null
  const scenarioTransitQueryChanged = (
    ...
    || request.scenario.policy.maxWalkKm !== request.maxWalkKm
    || request.scenario.policy.walkSpeedKph !== request.walkSpeedKph
  )
  if (scenarioTransitQueryChanged) {
    scenarioAccessibilityRange = await runAccessibilityRange({
      analysisStage: 'scenario-range',
      feedId: request.feedId,
      origin: request.origin,
      departMinutes: request.departMinutes,
      serviceDate: request.serviceDate,
      serviceDay: request.serviceDay,
      maxWalkKm: request.scenario.policy.maxWalkKm,
      walkSpeedKph: request.scenario.policy.walkSpeedKph,
      cutoffMinutes: request.cutoffsMinutes.at(-1),
    ...
    if (
      !scenarioAccessibilityRange
      || scenarioAccessibilityRange.schemaVersion !== 'vigo.analysis.accessibility-range.v1'
      || !Array.isArray(scenarioAccessibilityRange.stops)
    ) throw new Error('Scenario analysis received an invalid modified Accessibility range response.')
  }
  const scenarioServiceStops = Array.isArray(scenarioAccessibilityRange?.scenarioStops)
    ? scenarioAccessibilityRange.scenarioStops
    : []
  const scenarioTransitStops = Array.isArray(scenarioAccessibilityRange?.stops)
    ? scenarioAccessibilityRange.stops
    : transitStops
  const overlayDiagnostics = scenarioAccessibilityRange
    ?.diagnostics
    ?.timetable
  ...
  const propagation = {
    ...
    relaxations: scenarioAccessibilityRange?.diagnostics?.search?.relaxedStops ?? 0,
    ...
    queryMs: scenarioAccessibilityRange?.diagnostics?.search?.nativeQueryMs ?? 0,
    algorithm: scenarioAccessibilityRange?.diagnostics?.algorithm
  ...
  const rasterFromAccessibility = (range, label) => {
    ...
    if (
      ...
      const received = surface && typeof surface === 'object'
        ...
        : null
      throw new Error(
        `Scenario analysis received an invalid ${label} Accessibility surface: ${JSON.stringify(received)}.`,
      )
    }
    return {
  ...
  const baselineRaster = rasterFromAccessibility(accessibilityRange, 'baseline')
  ...
  const scenarioRaster = hasScenarioChanges
    ? rasterFromAccessibility(scenarioAccessibilityRange, 'scenario')
  ...
  const scenarioTerminalWalkMinutes = request.scenario.policy.maxWalkKm / request.scenario.policy.walkSpeedKph * 60
  ...
  const result = {
    ...
    summary: {
      ...
      transitStatus: accessibilityRange.diagnostics?.transit ?? null,
      scenarioTransitStatus: scenarioAccessibilityRange?.diagnostics?.transit
        ?? accessibilityRange.diagnostics?.transit
        ?? null,
    },
    surface: {
      reachability: {
        ...
        baselineTerminalWalkKm: request.maxWalkKm,
        scenarioTerminalWalkKm: request.scenario.policy.maxWalkKm,
        baselineTerminalWalkMinutes: Number(baselineTerminalWalkMinutes.toFixed(3)),
        scenarioTerminalWalkMinutes: Number(scenarioTerminalWalkMinutes.toFixed(3)),
      },
      raster: {
        width: request.rasterSize,
        height: request.rasterSize,
    ...
    limitations: scenarioEvidenceLimitations(request, hasScenarioChanges),
    diagnostics: {
      accessibilityDispatches: scenarioAccessibilityRange ? 2 : 1,
      streetConnectorDispatches: 0,
      analysisEngine: 'unified_native_one_to_many',
      accessibility: accessibilityRange.diagnostics ?? null,
      scenarioAccessibility: scenarioAccessibilityRange?.diagnostics ?? null,
      ...
      raster: {
        ...
        accessibilityTargets: transitStops.length,
      ...
      scenarioPropagation: {
        ...
        connectorStrategy: 'accessibility_owned_directed_osm_overlay_transfers',
      ...
      cache: {
        status: 'miss',
        key,
        surfaceOwner: 'accessibility-range',
      },
    },
  }
  const stored = cacheResult(key, result)
  result.diagnostics.cache = {
    ...result.diagnostics.cache,
    stored: stored.stored,
    weightBytes: stored.weightBytes,
    ...scenarioAnalysisCacheSnapshot(),
  }
